...
Это основная точка входа приложения для локальной разработки и тестирования
в режиме polling. Инициализирует все компоненты системы, подключает роутеры
обработчиков, настраивает логирование и мониторинг.

Режимы работы:
- **Polling mode**: Для локальной разработки (этот файл)
- **Webhook mode**: Для продакшн деплоя на Render (см. webhook_bot.py)

Основные возможности:
- Инициализация базы данных PostgreSQL
- Подключение всех обработчиков команд и сообщений
- Настройка AI сервиса (Google Gemini)
- Интеграция Sentry для отслеживания ошибок
- Система мониторинга здоровья приложения
- Фильтрация emoji из консольного вывода (Windows compatibility)
...
async def on_startup():
    ...
    if SENTRY_AVAILABLE and hasattr(settings, "sentry_dsn") and settings.sentry_dsn:
        sentry_sdk.init(
            ...
            if "render.com" in str(settings.database_url)
            else "development",
        )
