...
class TestPandaResponsesReal:
    ...
    async def test_no_empty_response_on_valid_question(self, ai_service):
        ...
        questions = [
            "Что такое скорость в физике? Объясни простыми словами.",
            "Объясни что такое подлежащее. Расскажи подробно.",
            "Когда началась Вторая мировая война? Расскажи подробно.",
        ]
        for q in questions:
            response = await ai_service.generate_response(
                user_message=q,
    ...
    async def test_cot_word_problem_apples(self, ai_service):
        ...
        """
        response = await ai_service.generate_response(
            user_message="В кафе было 23 яблока. 20 ушло на обед, купили ещё 6. Сколько всего яблок? Реши пошагово.",
            chat_history=[],
            user_age=10,
        )
        ...
        has_steps = any(
            p in response.lower()
            for p in ["сначала", "значит", "осталось", "23", "20", "6", "итого", "шаг"]
        )
        assert has_steps, f"Нет пошагового рассуждения: {response[:300]}"
        # Правильный ответ 9 (цифрой, словом или в контексте промежуточных вычислений)
        has_correct = (
            re.search(r"\b9\b", response) is not None
            or "девять" in response.lower()
        )
    ...
    async def test_subjects_math_history_biology_physics(self, ai_service):
        ...
        cases = [
            ("Математика", "Что такое квадратное уравнение? Объясни подробно.", ["уравнен", "квадрат", "x"]),
            ("История", "Когда началась Великая Отечественная война? Расскажи подробно.", ["1941", "войн", "герма"]),
            ("Биология", "Что такое клетка? Объясни подробно.", ["клетк", "организм", "ядро"]),
            ("Физика", "Что такое сила? Единица измерения? Объясни подробно.", ["сил", "ньютон", "н"]),
        ]
