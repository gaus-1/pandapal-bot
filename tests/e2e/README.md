...
- Полные пользовательские потоки - от регистрации до завершения задачи
- Интеграция всех компонентов - бот, API, БД, внешние сервисы
- Работа в реальных условиях - как будто настоящий пользователь
- Обработка ошибок на уровне приложения - что если что-то сломается
...
async def test_user_journey():
    ...
    # 5. Ход
    move_result = await make_move(game.id, "1,1")
...
- Требуют полной настройки окружения - БД, API ключи и т.д.
- Проверяют реальное поведение системы - как работает для пользователя
- Могут быть нестабильными - внешние зависимости могут быть недоступны
