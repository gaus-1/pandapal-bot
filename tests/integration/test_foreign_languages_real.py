...
def _check_real_api_key():
    ...
    try:
        ...
        if settings_key and settings_key != "test_api_key" and settings_key != "your_real_yandex_api_key_here" and len(settings_key) > 20:
            return True
    except Exception:
        pass
    return False
...
class TestForeignLanguageAIChat:
    ...
    async def test_english_text_with_translation_and_explanation(self):
        ...
        assert "перевод" in response.lower() or "столица" in response.lower() or "франция" in response.lower(), \
