...
class TestAISolidIntegration:
    ...
    def test_full_ai_flow_safe_content(self, mock_genai):
        """Тест полного потока AI для безопасного контента"""
        # Настройка моков
        mock_genai.configure = Mock()
        mock_model = Mock()
        mock_response = Mock()
        mock_response.text = "Отличный вопрос! Вот объяснение..."
        ...
        # Замена генератора в сервисе на наш
        service.response_generator = generator
        
        # Тест
        import asyncio
        result = asyncio.run(service.generate_response("Что такое фотосинтез?", user_age=10))
        
        assert "Отличный вопрос! Вот объяснение..." in result
        mock_model.generate_content_async.assert_called_once()
    
    @patch('bot.services.ai_response_generator_solid.genai')
    def test_full_ai_flow_unsafe_content(self, mock_genai):
        ...
        # Тест с небезопасным контентом
        import asyncio
        result = asyncio.run(service.generate_response("как курить", user_age=12))
        
        assert "Извините, но я не могу обсуждать эту тему" in result
        assert "Запрещенная тема" in result
    ...
    def test_solid_principles_compliance(self):
        ...
        from bot.services.ai_response_generator_solid import IModerator, IContextBuilder
    ...
    @patch('bot.services.ai_response_generator_solid.genai')
    def test_error_propagation(self, mock_genai):
        """Тест распространения ошибок через архитектуру"""
        # Настройка моков для генерации ошибки
        mock_genai.configure = Mock()
        mock_model = Mock()
        ...
        # Тест
        import asyncio
        result = asyncio.run(service.generate_response("Тест"))
        
        # Ошибка должна быть обработана и возвращен fallback ответ
