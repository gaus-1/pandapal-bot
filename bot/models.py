...
from sqlalchemy import (
    BigInteger, String, Integer, Boolean, Text,
    DateTime, JSON, CheckConstraint, ForeignKey, Index
)
...
class User(Base):
    ...
    __table_args__ = (
        CheckConstraint(
            "user_type IN ('child', 'parent', 'teacher')",
            name='ck_users_user_type'
        ),
        CheckConstraint(
            "age IS NULL OR (age >= 6 AND age <= 18)",
            name='ck_users_age_range'
        ),
        CheckConstraint(
            "grade IS NULL OR (grade >= 1 AND grade <= 11)",
            name='ck_users_grade_range'
        ),
    )
...
class LearningSession(Base):
    ...
    subject: Mapped[Optional[str]] = mapped_column(
        String(100),
        comment='Предмет (математика, русский и т.д.)'
    )
...
class UserProgress(Base):
    ...
    level: Mapped[Optional[int]] = mapped_column(
        Integer,
        comment='Текущий уровень владения (1-10)'
    )
    ...
    points: Mapped[int] = mapped_column(
        ...
        default=0,
        server_default='0',
        comment='Накопленные очки'
    )
    
    achievements: Mapped[Optional[dict]] = mapped_column(
        JSON,
        nullable=True,
        comment='Достижения (JSON: {achievement_id: date_earned})'
    )
