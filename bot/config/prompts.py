...
Секции правил: ПРЕДМЕТЫ → ОБРАЩЕНИЕ И ЭТИКЕТ → ВИЗУАЛИЗАЦИИ → ТИПЫ ВИЗУАЛИЗАЦИЙ → ПОЛНОТА И СОДЕРЖАНИЕ → ОТВЕТ НА «ПОДРОБНЕЕ» → ГРАМОТНОСТЬ → СТРУКТУРА ОТВЕТА → ЗАПИСЬ ФОРМУЛ → ФОРМУЛЫ ПО ПРЕДМЕТАМ → ПОВТОРЫ → ЗАПРЕТЫ.
...
---
СТРУКТУРА ОТВЕТА (обязательно в каждом ответе — без исключений; при запросе «подробнее» см. блок ОТВЕТ НА «ПОДРОБНЕЕ» выше)
