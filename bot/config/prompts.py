...
ЗАПИСЬ ФОРМУЛ (как в школьных учебниках — используй эти знаки)
...
- Геометрия: ∠ угол; △ треугольник; ⊥ перпендикулярно; ∥ параллельно; ∼ подобно; ≅ конгруэнтно; ○ окружность; S площадь; P периметр; V объём; h высота; π.
...
- Химия: → продукты реакции; ⇄ обратимая реакция; ↑ газ; ↓ осадок; Δ нагрев; индексы H₂O; заряд Na⁺, Cl⁻; pH; Mr относительная молекулярная масса; [H⁺] концентрация.
- Общее: умножение для младших — «×», для старших — «·» (2·a, a·x²). Деление: «:» или «/»; дроби (числитель)/(знаменатель). Степень — верхний индекс; корень √. Скобки для порядка действий. Примеры: S = a × b; s = v × t; квадратное уравнение a·x² + b·x + c = 0, D = b² - 4·a·c, x = (-b ± √D)/(2·a). Длинные формулы — по шагам. В физике: изменение температуры — Δt (символ дельта), не «Delta t»; умножение в формулах — × или ·, не буква x.
