...
from bot.services.analytics_service import AnalyticsService, AnalyticsPeriod
...
async def show_child_dashboard(callback_query, state: FSMContext):
    ...
    with get_db() as db:
        ...
        try:
            ...
            keyboard = [
                [
                    InlineKeyboardButton(text="📅 За неделю", callback_data=f"period_week_{child_id}"),
                    InlineKeyboardButton(text="📆 За месяц", callback_data=f"period_month_{child_id}")
                ],
                [
                    InlineKeyboardButton(text="📊 Детальный отчет", callback_data=f"detailed_{child_id}"),
                    InlineKeyboardButton(text="🔒 Настройки безопасности", callback_data=f"safety_{child_id}")
                ],
                [
...
async def show_overview_dashboard(callback_query, state: FSMContext):
    ...
    with get_db() as db:
        ...
        try:
            ...
            learning_analytics = await analytics_service.get_learning_analytics(AnalyticsPeriod.WEEK)
            ...
            text += f"• Средняя длительность: {learning_analytics.average_session_duration:.1f} мин\n"
        ...
        try:
            ...
            if user_analytics.learning_progress:
                ...
                for subject, progress in user_analytics.learning_progress.items():
                    text += f"• {subject}: Уровень {progress['level']}, {progress['points']} очков\n"
...
async def show_safety_settings(callback_query, state: FSMContext):
    ...
    with get_db() as db:
        ...
        try:
            ...
            keyboard = [
                [
                    InlineKeyboardButton(text="⚙️ Изменить настройки", callback_data=f"edit_safety_{child_id}"),
                    InlineKeyboardButton(text="📊 История активности", callback_data=f"activity_history_{child_id}")
                ],
                [
                    InlineKeyboardButton(text="⬅️ Назад", callback_data=f"dashboard_child_{child_id}")
                ]
...
async def generate_weekly_report(message: Message, state: FSMContext):
    ...
    with get_db() as db:
        ...
        try:
            ...
            await message.answer(
                text="❌ Ошибка генерации отчета. Попробуйте позже.",
                parse_mode="HTML"
            )
