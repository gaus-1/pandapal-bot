...
async def handle_ai_message(message: Message, state: FSMContext):  # noqa: ARG001
    ...
    try:
        ...
        with get_db() as db:
            ...
            # Для обычных текстовых запросов (без визуализации) системный промпт уже требует глубокие ответы,
            # но используем исходное сообщение без изменений, так как системный промпт покрывает все требования
            ...
            if visualization_image and visualization_type:
                ...
                if visualization_type == "multiplication_table":
                    enhanced_user_message = (
                        ...
                        "как её использовать (с конкретным примером), для чего она нужна, как читать таблицу. "
                        "Разбери несколько примеров использования таблицы. "
                        "Не перечисляй все значения из таблицы текстом — таблица уже показана. "
                        "Не упоминай графики, параболы или другие темы. Отвечай только на вопрос про таблицу умножения. "
                        "Минимум 4–5 предложений с примерами и пояснениями."
                    )
                elif visualization_type == "graph":
                    enhanced_user_message = (
                        f"{user_message}\n\n"
                        "Дай исчерпывающее глубокое пояснение про этот график: "
                        "что он показывает, как его читать (с конкретными примерами из графика), "
                        "основные свойства (возрастает/убывает, симметрия, нули, максимумы/минимумы). "
                        "Разбери несколько конкретных примеров из графика. "
                        "Не упоминай таблицы умножения или другие темы. Отвечай только на вопрос про график. "
                        "Минимум 4–5 предложений с примерами и пояснениями."
                    )
                    ...
                    enhanced_user_message = (
                        f"{user_message}\n\n"
                        f"Дай исчерпывающее глубокое пояснение про {diagram_name}: "
                        f"что она показывает, как её читать и использовать (с конкретными примерами), "
                        f"основные особенности и закономерности. Разбери несколько конкретных примеров из визуализации. "
                        f"Не дублируй визуализацию текстом (не перечисляй все значения). "
                        f"В конце ответа задай один вовлекающий вопрос (например: «Понятно? Могу объяснить подробнее?»). "
                        f"Минимум 5–6 предложений с примерами и пояснениями."
                    )
                elif visualization_type == "map":
                    enhanced_user_message = (
                        f"{user_message}\n\n"
                        "Дай исчерпывающее глубокое пояснение про карту: "
                        "что показывает карта, где находится объект, основные географические особенности, "
                        "соседние территории, водные объекты, важные достопримечательности. "
                        "Разбери несколько конкретных примеров из карты. Не дублируй карту текстом. "
                        "В конце ответа задай один вовлекающий вопрос. Минимум 5–6 предложений с примерами и пояснениями."
                    )
                else:
                    enhanced_user_message = (
                        f"{user_message}\n\n"
                        "Дай исчерпывающее глубокое пояснение только по теме запроса: "
                        "разбери тему подробно с примерами, объясни, как использовать визуализацию. "
                        "Не упоминай другие темы, которых не было в запросе. "
                        "В конце ответа задай один вовлекающий вопрос. Минимум 5–6 предложений с примерами и пояснениями."
                    )
        ...
        if visualization_image:
            ...
            # Проверяем, является ли это географическим вопросом
            # Если да - добавляем кнопку "Показать карту?"
            ...
            if geo_location:
                ...
                await message.answer(text=ai_response, reply_markup=map_keyboard)
            else:
