    ...
    def step(self, action: str) -> None:
        ...
        if self._can_place(new_row, new_col, new_rot):
            ...
            if action in ("down", "tick") and self.current_row >= 0:
                ...
                # КРИТИЧНО: Проверка всех блоков фигуры - блокируем если любой блок на дне или ниже
                # Это предотвращает выход блоков за границу
                blocks = self._get_blocks(self.current_row, self.current_col, self.current_rotation)
                # Проверяем ВСЕ блоки, включая те что могут быть за пределами доски
                max_row = max((r for r, c in blocks), default=-1)
                # Если максимальная строка >= height-1, блокируем
                # height-1 это последняя строка доски (индексация с 0)
