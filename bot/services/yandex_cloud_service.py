...
class YandexCloudService:
    ...
    async def analyze_image_with_text(
        ...
        try:
            ...
            # Шаг 2: Анализируем через YandexGPT
            analysis_prompt = f"""
            На изображении распознан текст: "{recognized_text}"

            Вопрос ученика: {user_question or "Помоги разобраться с этим заданием"}

            Проанализируй как учитель:
            1. О чем это задание?
            2. Какие шаги нужны для решения?
            3. Дай подсказку (не полное решение!)
            """

            gpt_analysis = await self.generate_text_response(
                user_message=analysis_prompt,
                system_prompt="Ты - опытный учитель. Помогай ученикам разобраться, но не давай готовых ответов.",
                temperature=0.6,
            )
