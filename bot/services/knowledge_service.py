...
from bot.services.web_scraper import WebScraperService, EducationalContent
...
class KnowledgeService:
    ...
    def __init__(self):
        ...
        self.update_interval = timedelta(hours=24)  # Обновляем раз в день
        
        logger.info("📚 KnowledgeService инициализирован")
    
    async def get_knowledge_for_subject(self, subject: str, query: str = "") -> List[EducationalContent]:
        """
        ...
        if query:
            ...
            subject_materials = [
                material for material in subject_materials
                if query_lower in material.title.lower() or query_lower in material.content.lower()
            ]
        
        return subject_materials
    
    async def get_helpful_content(self, user_question: str, user_age: Optional[int] = None) -> List[EducationalContent]:
        """
        ...
        for subject, materials in self.knowledge_base.items():
            for material in materials:
                ...
                if (question_lower in material.title.lower() or 
                    question_lower in material.content.lower() or
                    self._is_question_related_to_subject(question_lower, subject)):
    ...
    def _is_question_related_to_subject(self, question: str, subject: str) -> bool:
        ...
        subject_keywords = {
            "matematika": ["математика", "число", "решить", "задача", "уравнение", "формула", "считать", "плюс", "минус", "умножить", "делить", "дробь", "процент", "геометрия", "алгебра"],
            "russkiy-yazyk": ["русский", "язык", "слово", "предложение", "грамматика", "орфография", "пунктуация", "часть речи", "существительное", "прилагательное", "глагол", "написать", "сочинение", "изложение"],
            "literatura": ["литература", "книга", "писатель", "поэт", "стихотворение", "рассказ", "роман", "произведение", "автор", "герой", "сюжет", "тема", "идея"],
            "istoriya": ["история", "исторический", "война", "революция", "царь", "император", "древний", "средние века", "современность", "дата", "событие", "персона"],
            "geografiya": ["география", "страна", "город", "столица", "материк", "океан", "река", "гора", "климат", "население", "карта", "координаты"],
            "biologiya": ["биология", "животное", "растение", "клетка", "орган", "система", "размножение", "эволюция", "экосистема", "природа", "человек", "анатомия"],
            "fizika": ["физика", "сила", "энергия", "скорость", "масса", "температура", "давление", "электричество", "магнетизм", "свет", "звук", "механика"],
            "khimiya": ["химия", "химический", "элемент", "реакция", "вещество", "молекула", "атом", "периодическая", "таблица", "кислота", "щелочь", "соль"],
            "informatika": ["информатика", "компьютер", "программа", "алгоритм", "код", "программирование", "интернет", "сайт", "данные", "файл", "система"],
        }
    ...
    async def update_knowledge_base(self):
        ...
        try:
            ...
            async with WebScraperService() as scraper:
                ...
                logger.info(f"✅ База знаний обновлена: {len(all_materials)} материалов по {len(self.knowledge_base)} предметам")
