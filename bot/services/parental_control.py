...
class ParentalControlService:
    ...
    def _get_parents_of_child(self, child_id: int) -> List[User]:
        ...
        parent = self.db.query(User).filter(
            User.telegram_id == child.parent_telegram_id,
    ...
    def _get_message_statistics(self, child_id: int, start: datetime, end: datetime) -> Dict[str, int]:
        """Получает статистику сообщений ребенка"""
        # Запрос к таблице chat_history
        total_messages = self.db.query(ChatHistory).filter(
        ...
        return {
            ...
            "blocked": 0,  # TODO: получить из логов модерации
            "suspicious": 0,  # TODO: получить из логов
            "ai_interactions": total_messages,  # Каждое сообщение = взаимодействие с AI
            "voice_messages": 0  # TODO: получить из логов
        }

    def _get_moderation_statistics(self, child_id: int, start: datetime, end: datetime) -> Dict[str, int]:
        """Получает статистику модерации"""
        # Здесь можно получить данные из логов модерации
    ...
    def _get_recent_activities(self, child_id: int, start: datetime, end: datetime) -> List[ActivityRecord]:
    ...
    def _generate_recommendations(self, message_stats: Dict, moderation_stats: Dict) -> List[str]:
        ...
        if message_stats["total"] > 50:
            recommendations.append("Ребенок активно использует бота. Рекомендуется обсудить баланс между обучением и развлечением.")
        
        # Рекомендации по заблокированному контенту
        blocked_count = message_stats["blocked"]
        if blocked_count > 5:
            recommendations.append("Обнаружено много заблокированных сообщений. Рекомендуется поговорить с ребенком о правилах безопасности.")
        
        # Рекомендации по подозрительной активности
        if message_stats["suspicious"] > 0:
            recommendations.append("Обнаружена подозрительная активность. Рекомендуется усиленный контроль.")
    ...
    async def link_parent_to_child(self, parent_id: int, child_id: int) -> bool:
        ...
        try:
            ...
            if not parent or not child:
                logger.error(f"❌ Родитель или ребенок не найдены: parent={parent_id}, child={child_id}")
                return False
    ...
    async def get_parental_control_stats(self) -> Dict[str, Any]:
        ...
        linked_children = self.db.query(User).filter(
            User.user_type == "child",
            User.parent_telegram_id.isnot(None)
        ).count()
        ...
        return {
            ...
            "coverage_percentage": (linked_children / total_children * 100) if total_children > 0 else 0
        }
