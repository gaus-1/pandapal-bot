...
class ContentModerationService(IModerationService):
    ...
    def is_safe_content(self, text: str) -> tuple[bool, str | None]:
        ...
        # Дополнительная проверка: если контент содержит учебные слова - разрешаем ДО проверки на запрещенные темы
        # Это позволяет пропускать образовательные запросы даже если они содержат слова, которые могут быть в запрещенных паттернах
