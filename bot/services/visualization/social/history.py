...
class HistoryVisualization(BaseVisualizationService):
    ...
    def _draw_borodino_battle(self, ax):
        ...
        ax.fill_between(river_x, [y - 0.3 for y in river_y], [y + 0.3 for y in river_y],
                        color="#64b5f6", alpha=0.7)
        ax.text(1, 5.5, "р. Колоча", fontsize=9, color="blue", style="italic")

        # Русские позиции (красные)
        ax.add_patch(Rectangle((1, 2), 3, 1.5, facecolor="#ef5350", edgecolor="darkred", linewidth=2))
        ax.text(2.5, 2.75, "Русская армия\n(Кутузов)", ha="center", va="center", fontsize=9, fontweight="bold")

        # Флеши Багратиона
        for i, x in enumerate([2, 3, 4]):
            ax.add_patch(Polygon([[x, 1.5], [x + 0.3, 1.8], [x + 0.6, 1.5]],
                                 facecolor="#c62828", edgecolor="black"))
        ax.text(3.3, 1.2, "Флеши Багратиона", fontsize=8, ha="center")

        # Батарея Раевского
        ax.add_patch(Rectangle((5.5, 2.5), 1.5, 0.8, facecolor="#c62828", edgecolor="black", linewidth=2))
        ax.text(6.25, 2.9, "Батарея\nРаевского", ha="center", va="center", fontsize=8)

        # Французские позиции (синие)
        ax.add_patch(Rectangle((1, 6.5), 4, 1.5, facecolor="#42a5f5", edgecolor="darkblue", linewidth=2))
        ax.text(3, 7.25, "Французская армия\n(Наполеон)", ha="center", va="center", fontsize=9, fontweight="bold")

        # Стрелки атак
        ax.annotate("", xy=(2.5, 4), xytext=(2.5, 6.5),
                    arrowprops=dict(arrowstyle="->", color="blue", lw=2))
        ax.annotate("", xy=(6, 4), xytext=(5, 6.5),
                    arrowprops=dict(arrowstyle="->", color="blue", lw=2))
        ...
        ax.annotate("", xy=(9.5, 6.1), xytext=(9, 6.1),
                    arrowprops=dict(arrowstyle="->", color="blue", lw=2))
    ...
    def _draw_kulikovo_battle(self, ax):
        ...
        # Засадный полк
        ax.add_patch(Rectangle((10, 1), 1.5, 0.8, facecolor="#c62828", edgecolor="black", linewidth=2))
        ax.text(10.75, 1.4, "Засадный\nполк", ha="center", va="center", fontsize=7, fontweight="bold")

        # Ордынцы
        ax.add_patch(Rectangle((3, 6.5), 6, 1.5, facecolor="#ffc107", edgecolor="#f57c00", linewidth=2))
        ax.text(6, 7.25, "Войско Мамая\n(Золотая Орда)", ha="center", va="center", fontsize=9, fontweight="bold")

        # Стрелки
        ax.annotate("", xy=(6, 3), xytext=(6, 6.5),
                    arrowprops=dict(arrowstyle="->", color="#f57c00", lw=2))
        ax.annotate("", xy=(6, 6.5), xytext=(10.5, 2),
    ...
    def _draw_poltava_battle(self, ax):
        ...
        # Русские редуты
        for i in range(6):
            ax.add_patch(Rectangle((3 + i * 0.8, 4), 0.6, 0.4, facecolor="#c62828", edgecolor="black"))
        ax.text(5.5, 3.5, "Редуты", fontsize=8, ha="center")

        # Русская армия
        ax.add_patch(Rectangle((2, 1), 5, 1.5, facecolor="#ef5350", edgecolor="darkred", linewidth=2))
        ax.text(4.5, 1.75, "Русская армия\n(Пётр I)", ha="center", va="center", fontsize=9, fontweight="bold")

        # Шведская армия
        ax.add_patch(Rectangle((2, 6.5), 5, 1.5, facecolor="#1565c0", edgecolor="darkblue", linewidth=2))
        ax.text(4.5, 7.25, "Шведская армия\n(Карл XII)", ha="center", va="center", fontsize=9, fontweight="bold")

        # Стрелки атак
        ax.annotate("", xy=(4.5, 4.5), xytext=(4.5, 6.5),
                    arrowprops=dict(arrowstyle="->", color="blue", lw=2))
    ...
    def _draw_stalingrad_battle(self, ax):
        ...
        ax.add_patch(Circle((6, 4.5), 2, facecolor="#424242", edgecolor="black", linewidth=2, alpha=0.7))
    ...
    def _draw_ledovoe_battle(self, ax):
        ...
        # Чудское озеро
        ax.text(6, 8.5, "Чудское озеро (лёд)", fontsize=11, ha="center", style="italic", color="blue")
        ...
        # Русские войска
        ax.add_patch(Rectangle((4, 2), 4, 1.5, facecolor="#ef5350", edgecolor="darkred", linewidth=2))
        ax.text(6, 2.75, "Русское войско\n(Александр Невский)", ha="center", va="center", fontsize=9)
        ...
        ax.add_patch(Rectangle((9.5, 3), 1.5, 1, facecolor="#c62828", edgecolor="black", linewidth=2))
        ...
        # Стрелки
        ax.annotate("", xy=(6, 4), xytext=(6, 6),
                    arrowprops=dict(arrowstyle="->", color="gray", lw=2))
        ax.annotate("", xy=(4.5, 5), xytext=(2, 4),
                    arrowprops=dict(arrowstyle="->", color="red", lw=2, connectionstyle="arc3,rad=0.2"))
        ax.annotate("", xy=(7.5, 5), xytext=(10, 4),
                    arrowprops=dict(arrowstyle="->", color="red", lw=2, connectionstyle="arc3,rad=-0.2"))
