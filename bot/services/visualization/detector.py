...
class VisualizationDetector:
    ...
    def detect_geography_question(self, text: str) -> str | None:
        ...
        Returns:
            str | None: Название страны/города для карты или None если это не географический вопрос
        """
        ...
        # Используем [а-яёА-ЯЁa-zA-Z]+ вместо \w+ для поддержки кириллицы
        geo_patterns = [
            r"где\s+находится\s+([а-яёА-ЯЁa-zA-Z]+)",
            r"где\s+расположен[аоы]?\s+([а-яёА-ЯЁa-zA-Z]+)",
            r"в\s+какой\s+части\s+(?:мира|света)\s+находится\s+([а-яёА-ЯЁa-zA-Z]+)",
            r"на\s+каком\s+(?:континенте|материке)\s+находится\s+([а-яёА-ЯЁa-zA-Z]+)",
            r"расскажи\s+(?:про\s+)?(?:где\s+)?([а-яёА-ЯЁa-zA-Z]+)\s+(?:находится|расположен)",
            r"расскажи\s+(?:мне\s+)?про\s+([а-яёА-ЯЁa-zA-Z]+)",  # "расскажи про Германию"
            r"расскажи\s+(?:мне\s+)?о\s+([а-яёА-ЯЁa-zA-Z]+)",  # "расскажи о Китае"
            r"что\s+(?:ты\s+)?знаешь\s+(?:о|про)\s+([а-яёА-ЯЁa-zA-Z]+)",  # "что знаешь о Китае"
        ]
        ...
        for pattern in geo_patterns:
            ...
            if match:
                ...
                # Проверяем, что это действительно страна/город
                # Нормализуем название и убираем падежные окончания
                location_normalized = location.lower().strip()

                # Убираем типичные падежные окончания (винительный, родительный, предложный)
                for ending in ["ию", "ии", "ей", "ью", "ой", "ый", "ая", "ое", "ах", "ях"]:
                    if location_normalized.endswith(ending) and len(location_normalized) > 4:
                        ...
                        # Добавляем типичное окончание именительного падежа
                        if ending in ["ию", "ии"]:
                            location_normalized += "ия"
                        elif ending in ["ей", "ью"]:
                            location_normalized += "ь"
                        elif ending == "ой":
                            location_normalized += "а"
                        break

                # Проверяем в нашем списке стран и городов
                known_locations = [
                    # Страны
                    "китай",
                    "россия",
                ...
                for loc in known_locations:
                    if loc in location_normalized or location_normalized in loc:
                        logger.info(f"🗺️ Обнаружен географический вопрос о: {location}")
                        return location
    ...
    def detect(self, text: str) -> tuple[bytes | None, str | None]:
        ...
        - Если запрос "где находится X" - НЕ генерировать карту (будет кнопка)
        ...
        visualization_request_words = list(VISUALIZATION_TRIGGER_WORDS) + [
            ...
            "подготовка к огэ",
            "подготовка к впр",
            # УЛУЧШЕНО: Добавлены универсальные детские формулировки
            "скинь картинку",
            "есть рисунок",
            "объясни наглядно",
            ...
            "нарисуй карту",
            "создай карту",
            # УЛУЧШЕНО: Добавлены ключевые слова для графиков из примера
            "график",
            "зависимость",
            "прямая",
            "кривая",
            ...
            "наклонная",
            "линия тренда",
            # УЛУЧШЕНО: Добавлены детские формулировки и сленг
            "параболка",
            "прямая пропорция",
            "график с дробями",
            "сколько проедем",
            ...
            "цена товара",
            "часть от целого",
            # УЛУЧШЕНО: Математика - детские формулировки
            "нарисуй квадрат",
            ...
            # УЛУЧШЕНО: Геометрия - детские формулировки
            "нарисуй треугольник",
            ...
            "чертеж фигуры",
            "синус косинус",
            # УЛУЧШЕНО: Добавлены ключевые слова для диаграмм
            "диаграмма",
            "столбчатая",
            "круговая",
            "столбик",
            "сектор",
            "климатограмма",
            # УЛУЧШЕНО: Добавлены ключевые слова для карт
            "схема местности",
            "карта полушарий",
            "физическая карта",
            "политическая карта",
            "топограф",
            # УЛУЧШЕНО: Добавлены ключевые слова для схем/чертежей
            "схема",
            "чертеж",
            "рисунок",
            "изображение",
            ...
            "план",
            "разрез",
            # УЛУЧШЕНО: Добавлены детские формулировки
            "картинка",
            "фотка",
            "схемка",
            "табличка",
            # УЛУЧШЕНО: Добавлены ключевые слова для таблиц
            "расписание",
            "сводная",
            "решето пеннета",
            "периодическая система",
            # Математика (алгебра/геометрия) - расширенный список
            "функция",
            ...
            "границы государств",
            "торговые пути",
            # Обществознание - расширенный список
            "структура общества",
            "сферы общества",
            "социальные группы",
            "ветви власти",
            ...
            # Информатика - расширенный список
            "блок-схема",
            "блок схема",
            "алгоритм",
            "таблица истинности",
            ...
            "компьютерная сеть",
            "схема сети",
            # Русский язык - расширенный список
            "схема предложения",
            "схема слова",
            "звуковая схема",
            "разбор слова",
        ...
        # КРИТИЧНО: Сначала проверяем СПЕЦИФИЧНЫЕ схемы (если есть запрос визуализации)
        # Это должно быть ПЕРЕД проверкой универсальных диаграмм, чтобы специфичные запросы обрабатывались раньше
        if has_visualization_request:
            # ПРИОРИТЕТ: график параболы — учитываем ВСЕ слова запроса (нарисуй + график + параболы)
            if re.search(r"график\s+парабол|график\s+порабол|парабол[аы]|порабол[аы]", text_lower):
                image = self.viz_service.generate_function_graph("x**2")
                if image:
                    logger.info("📈 Детектирован график параболы (приоритетная проверка)")
                    return image, "graph"

            # Проверяем запросы на специфичные схемы ПЕРЕД общими диаграммами
            # Схема Солнечной системы
            astronomy_scheme_patterns = [
                r"схем[аеыу]?\s+планет",
                r"покажи\s+схем[у]?\s+планет",
                r"нарисуй\s+схем[у]?\s+планет",
                r"схем[аеыу]?\s+солнечн[ая]?\s+систем[ы]",
                r"планет[ы]?\s+в\s+порядк[е]",
                r"какие\s+планет[ы]?\s+есть",
            ]
            for pattern in astronomy_scheme_patterns:
                if re.search(pattern, text_lower):
                    image = self.viz_service.generate_solar_system_scheme()
                    if image:
                        logger.info("📈 Детектирована схема планет/Солнечной системы")
                        return image, "scheme"

            # Схема строения тела человека
            biology_scheme_patterns = [
                r"схем[аеыу]?\s+строени[яе]?\s+тел[аа]?\s+человек[а]",
                r"строени[е]?\s+тел[аа]?\s+человек[а]",
                r"внутренност[и]?\s+человек[а]",
                r"покажи\s+схем[у]?\s+тел[аа]?\s+человек[а]",
                r"нарисуй\s+схем[у]?\s+тел[аа]?\s+человек[а]",
                r"анатоми[я]?\с+человек[а]",
                r"орган[ы]?\s+человек[а]",
                r"схем[аеыу]?\s+кровообращени[яе]",
                r"схем[аеыу]?\s+строени[яе]?\s+клетк[и]",
                r"схем[аеыу]?\s+дыхани[яе]",
            ]
            for pattern in biology_scheme_patterns:
                if re.search(pattern, text_lower):
                    image = self.viz_service.generate_human_body_structure_scheme()
                    if image:
                        logger.info("📈 Детектирована схема строения тела человека")
                        return image, "scheme"

            # 36.8. Окружающий мир: круговорот воды
            water_cycle_patterns = [
                r"круговорот[а]?\s+вод[ы]",
                r"схем[аеыу]?\s+круговорот[а]?\s+вод[ы]",
                r"как\s+движется\s+вод[а]",
                r"испарени[е]?\s+вод[ы]",
                r"круговорот[а]?\s+вод[ы]?\s+в\s+природ[е]",
            ]
            for pattern in water_cycle_patterns:
                if re.search(pattern, text_lower):
                    image = self.viz_service.generate_water_cycle_scheme()
                    if image:
                        logger.info("📈 Детектирован круговорот воды")
                        return image, "scheme"

            # 36.9. Биология: строение клетки
            cell_patterns = [
                r"схем[аеыу]?\s+строени[яе]?\s+клетк[и]",
                r"строени[е]?\s+клетк[и]",
                r"клеточн[ая]?\s+мембран[а]",
                r"ядр[о]?\s+клетк[и]",
                r"органелл[ы]?\s+клетк[и]",
            ]
            for pattern in cell_patterns:
                if re.search(pattern, text_lower):
                    image = self.viz_service.generate_cell_structure_scheme()
                    if image:
                        logger.info("📈 Детектирована схема строения клетки")
                        return image, "scheme"

            # 36.10. Биология: строение ДНК
            dna_patterns = [
                r"схем[аеыу]?\s+строени[яе]?\s+днк",
                r"строени[е]?\s+днк",
                r"двойн[ая]?\s+спираль",
                r"молекул[а]?\s+днк",
                r"структур[а]?\s+днк",
            ]
            for pattern in dna_patterns:
                if re.search(pattern, text_lower):
                    image = self.viz_service.generate_dna_structure_scheme()
                    if image:
                        logger.info("📈 Детектирована схема строения ДНК")
                        return image, "scheme"

            # 36.11. Информатика: блок-схема алгоритма
            flowchart_patterns = [
                r"блок[-\s]?схем[аеыу]",
                r"схем[аеыу]?\s+алгоритм[а]",
                r"блок[-\s]?схем[аеыу]?\s+алгоритм[а]",
                r"алгоритм\s+в\s+виде\s+схем[ы]",
                r"графическ[ое]?\s+представлени[е]?\s+алгоритм[а]",
            ]
            for pattern in flowchart_patterns:
                if re.search(pattern, text_lower):
                    image = self.viz_service.generate_algorithm_flowchart_scheme()
                    if image:
                        logger.info("📈 Детектирована блок-схема алгоритма")
                        return image, "scheme"

            # 36.12. Обществознание: структура государства
            state_patterns = [
                r"схем[аеыу]?\s+структур[ы]?\s+государств[а]",
                r"структур[а]?\s+государств[а]",
                r"ветв[и]?\s+власт[и]",
                r"разделени[е]?\s+власт[ей]",
                r"законодательн[ая]?\s+исполнительн[ая]?\s+судебн[ая]?\s+власт[ь]",
            ]
            for pattern in state_patterns:
                if re.search(pattern, text_lower):
                    image = self.viz_service.generate_state_structure_scheme()
                    if image:
                        logger.info("📈 Детектирована схема структуры государства")
                        return image, "scheme"
            ...
            # УЛУЧШЕНО: Добавлены паттерны для запросов типа "покажи диаграмму" без указания типа
            ...
            # УЛУЧШЕНО: Проверяем общие запросы на диаграмму (без указания типа)
            # Например: "нарисуй задачу и покажи диаграмму" - должна генерироваться круговая диаграмма
            # ИСКЛЮЧАЕМ: Не срабатываем на "схему" (это схемы, не диаграммы)
            # ШКОЛЬНЫЕ ЗАПРОСЫ: "доли", "части", "процент", "соотношение" → круговая диаграмма
            general_diagram_patterns = [
                ...
                # Школьные запросы про доли и проценты (часто используют круговые)
                r"дол[июя].*диаграмм",
                r"част[иья].*диаграмм",
                r"процент.*диаграмм",
            ...
            # КРИТИЧНО: Если в запросе есть "схем" (схема, схемы, схему), это НЕ диаграмма
            # Проверяем перед генерацией, чтобы не перехватывать запросы на схемы
            if "схем" in text_lower:
                has_general_diagram_request = False

            # Если есть общий запрос на диаграмму без указания типа - генерируем круговую (pie)
            if has_general_diagram_request and not any(
                keyword in text_lower for keyword in diagram_type_patterns
            ):
                # Генерируем круговую диаграмму с демонстрационными данными
                try:
                    demo_data = {
                        "Математика": 30,
                        "Русский": 25,
                    ...
                    image = self.viz_service.generate_pie_chart(demo_data, "Диаграмма")
                    if image:
                        logger.info(
                            "📊 Детектирован общий запрос на диаграмму, сгенерирована круговая"
                        )
                        return image, "pie"
        ...
        # 31. География: карты стран через Yandex Maps Static API
        # ВАЖНО: Запросы "где находится X" НЕ генерируют карту сразу!
        # Для них будет кнопка "Показать карту?" после текстового ответа.
        # Карта генерируется ТОЛЬКО для прямых запросов: "покажи", "нарисуй", "карта X"

        # Сначала проверяем - это вопрос "где находится"? Если да - пропускаем генерацию карты
        if not self.detect_geography_question(text):
            # Это прямой запрос на карту - генерируем сразу
            map_keywords = [
                # Запросы на карты стран, городов, районов (БЕЗ "где находится")
                r"карт[аеыу]?\s+(?:страны|мира|япони|росси|кита|сша|франци|германи|великобритани|инди|бразили|австрали|канад|итали|испани|египт|мексик|европ|турци|польш|украин|казахстан|узбекистан|монголи|таиланд|вьетнам|индонези|филиппин|малайзи|сингапур|саудовск|израил|иран|ирак|пакистан|бангладеш|афганистан|аргентин|чили|перу|колумби|венесуэл|куб|южн[ая]?\s+африк|нигери|кени|эфиопи|марокк|алжир|тунис|ливи|судан|питер|петербург|санкт[-\s]?петербург|спб|москв|мск|район)",
                # Запросы типа "покажи спб", "покажи москву", "покажи район"
                r"(?:покажи|нарисуй|создай|составь|построй|выведи|отобрази)\s+(?:карт[аеыу]?\s+)?(?:япони|росси|кита|сша|франци|германи|великобритани|инди|бразили|австрали|канад|итали|испани|египт|мексик|европ|турци|польш|украин|казахстан|узбекистан|монголи|таиланд|вьетнам|индонези|филиппин|малайзи|сингапур|саудовск|израил|иран|ирак|пакистан|бангладеш|афганистан|аргентин|чили|перу|колумби|венесуэл|куб|южн[ая]?\s+африк|нигери|кени|эфиопи|марокк|алжир|тунис|ливи|судан|питер|петербург|санкт[-\s]?петербург|спб|москв|мск|район)",
                r"(?:покажи|нарисуй|создай|составь|построй|выведи|отобрази)\s+(?:карт[аеыу]|местоположени)",
                r"покажи\s+на\s+карте",  # "покажи на карте"
                r"физическ[ая]?\s+карт[аеыу]?\s+росси",  # "физическая карта России"
                r"гор[ы]?\s+африк",  # "горы Африки"
            ]

            for pattern in map_keywords:
                match = re.search(pattern, text_lower)
                if match:
                    # Извлекаем название страны/города/района из текста
                    # Приоритет: район > город > страна
                    # Сначала проверяем запросы на административные округа Москвы
                    okrug_match = re.search(
                        r"(цао|сао|свао|вао|ювао|юао|юзао|зао|сзао|центральн\w*\s*(?:административн\w*)?\s*округ|северн\w*\s*(?:административн\w*)?\s*округ|северо[-\s]?восточн\w*\s*(?:административн\w*)?\s*округ|восточн\w*\s*(?:административн\w*)?\s*округ|юго[-\s]?восточн\w*\s*(?:административн\w*)?\s*округ|южн\w*\s*(?:административн\w*)?\s*округ|юго[-\s]?западн\w*\s*(?:административн\w*)?\s*округ|западн\w*\s*(?:административн\w*)?\s*округ|северо[-\s]?западн\w*\s*(?:административн\w*)?\s*округ)",
                        text_lower,
                    )
                    if okrug_match and "москв" in text_lower:
                        ...
                        okrug_map = {
                            ...
                            "северн": "северный",
                            "свао": "свао",
                            "северо-восточн": "северо-восточный",
                            "вао": "вао",
                            "восточн": "восточный",
                            "ювао": "ювао",
                            "юго-восточн": "юго-восточный",
                            "юао": "юао",
                            "южн": "южный",
                            "юзао": "юзао",
                            "юго-западн": "юго-западный",
                            "зао": "зао",
                            "западн": "западный",
                            "сзао": "сзао",
                            "северо-западн": "северо-западный",
                        }
                        for key, val in okrug_map.items():
                            if key in okrug_name:
                                okrug_name = val
                                break
                        image = self.viz_service.generate_country_map(okrug_name)
                        if image:
                            logger.info(f"🗺️ Детектирован округ Москвы: {okrug_name}")
                            return image, "map"

                    # Затем проверяем запросы на районы СПб и Москвы
                    district_match = re.search(
                        r"(московский|адмиралтейский|василеостровский|выборгский|калининский|кировский|колпинский|красногвардейский|красносельский|кронштадтский|курортный|невский|петроградский|петродворцовый|приморский|пушкинский|фрунзенский|центральный|арбат|тверской|хамовники|замоскворечье|китай[-\s]?город|басманный|таганский|пресненский|мещанский)\s*(?:район)?\s*(?:спб|петербург|питер|москв)?",
                        text_lower,
                    )
                    if district_match:
                        district_name = district_match.group(1)
                        if any(word in text_lower for word in ["спб", "петербург", "питер"]):
                            country_name = f"{district_name} район спб"
                        elif "москв" in text_lower or district_name in [
                            "арбат",
                            "тверской",
                            "хамовники",
                            "замоскворечье",
                        ...
                        if image:
                            logger.info(f"🗺️ Детектирован район: {country_name}")
                            return image, "map"

                    # Затем проверяем города (включая аббревиатуры: мск, спб)
                    city_match = re.search(
                        r"\b(питер|петербург|санкт[-\s]?петербург|спб|москв[ауеы]?|мск)\b",
                        text_lower,
                    )
                    if city_match:
                        city_name = city_match.group(1)
                        if city_name in ["питер", "петербург", "спб"] or "санкт" in city_name:
                            country_name = "санкт-петербург"
                        elif "москв" in city_name or city_name == "мск":
                            country_name = "москва"
                        else:
                            country_name = city_name
                        image = self.viz_service.generate_country_map(country_name)
                        if image:
                            logger.info(f"🗺️ Детектирован город: {country_name}")
                            return image, "map"

                    # Затем проверяем страны
                    country_match = re.search(
                        r"(япони|росси|кита|сша|франци|германи|великобритани|инди|бразили|австрали|канад|итали|испани|египт|мексик|европ|страна|мир|турци|польш|украин|казахстан|узбекистан|монголи|таиланд|вьетнам|индонези|филиппин|малайзи|сингапур|саудовск|израил|иран|ирак|пакистан|бангладеш|афганистан|аргентин|чили|перу|колумби|венесуэл|куб|южн[ая]?\s+африк|нигери|кени|эфиопи|марокк|алжир|тунис|ливи|судан)",
                        text_lower,
                    )
                    if country_match:
                        country_name = country_match.group(1)
                        if "европ" in country_name:
                            country_name = "европа"
                        else:
                            full_country_match = re.search(
                                rf"({country_match.group(1)}[а-я]*)", text_lower
                            )
                            if full_country_match:
                                country_name = full_country_match.group(1)
                        image = self.viz_service.generate_country_map(country_name)
                        if image:
                            logger.info(f"🗺️ Детектирована страна: {country_name}")
                            return image, "map"

                    # Если ничего не найдено - показываем карту мира
                    if "карт" in text_lower or "покажи" in text_lower:
                        image = self.viz_service.generate_country_map("мир")
                        if image:
                            logger.info("🗺️ Детектирована карта мира (по умолчанию)")
                            return image, "map"

        # 32. История: хронология
        # УЛУЧШЕНО: Добавлены детские формулировки
        history_patterns = [
        ...
        for pattern in history_patterns:
            if re.search(pattern, text_lower):
                ...
                if "схем" in text_lower and "битв" in text_lower:
                    ...
                    for b in battles:
                        if b in text_lower:
                            battle = b
                            break
                    image = self.viz_service.generate_battle_scheme(battle)
                    if image:
                ...
                # Проверяем запрос на хронологию войны
                    ...
                    if (
                        "1812" in text_lower
                        or "наполеон" in text_lower
                        or "отечественн" in text_lower
                    ):
                        war = "1812"
                    elif "северн" in text_lower or "швец" in text_lower:
                        war = "северная"
                    image = self.viz_service.generate_war_timeline(war)
                    if image:
        ...
        # УЛУЧШЕНО: Добавлены паттерны из примера для лучшей детекции
        # УЛУЧШЕНО: Добавлены детские формулировки
        ...
        # УЛУЧШЕНО: Добавлены паттерны из примера
        ...
        # 36.5. Геометрия: медиана треугольника
        ...
        # 36.5. Астрономия: схема планет (Солнечная система)
        # УЛУЧШЕНО: Добавлена генерация схемы Солнечной системы
        astronomy_scheme_patterns = [
            r"схем[аеыу]?\s+планет",
            r"покажи\s+схем[у]?\s+планет",
            r"нарисуй\s+схем[у]?\s+планет",
        ...
        # 36.6. Биология: схемы строения (кровообращение, клетка, органы, тело человека)
        # УЛУЧШЕНО: Добавлена генерация схемы строения тела человека
        biology_scheme_patterns = [
            r"схем[аеыу]?\s+строени[яе]?\s+тел[аа]?\s+человек[а]",
            r"строени[е]?\s+тел[аа]?\s+человек[а]",
            r"внутренност[и]?\s+человек[а]",
            ...
            if re.search(pattern, text_lower):
                ...
                for z in [
                    ...
                    if z in text_lower:
                        zone = z
                        break
                image = self.viz_service.generate_climatogram(zone)
                if image:
                    logger.info(f"📊 Детектирована климатограмма: {zone}")
                    return image, "graph"

        # 36.8. Информатика: блок-схемы
        flowchart_patterns = [
            r"блок[-\s]?схем[аеыу]",
            r"схем[аеыу]?\s+алгоритм[а]",
            r"алгоритм\s+в\s+виде\s+схем[ы]",
        ...
        for pattern in flowchart_patterns:
            if re.search(pattern, text_lower):
                ...
                if re.search(r"ветвлени|если|условн", text_lower):
                    ...
                    alg_type = "factorial"
                image = self.viz_service.generate_flowchart(alg_type)
                if image:
                    logger.info(f"📊 Детектирована блок-схема: {alg_type}")
                    return image, "scheme"

        # 36.9. Информатика: таблицы истинности
        ...
        for pattern in truth_table_patterns:
            if re.search(pattern, text_lower):
                ...
                if re.search(r"\bили\b|or", text_lower):
                    ...
                    operation = "xor"
                image = self.viz_service.generate_truth_table(operation)
                if image:
                    logger.info(f"📊 Детектирована таблица истинности: {operation}")
                    return image, "table"

        # 37. Физика: закон Ома и электрические схемы
        # ИСПРАВЛЕНО: Ужесточены паттерны, чтобы не срабатывать на случайные слова с "ом"
        # УЛУЧШЕНО: Добавлены паттерны из примера
        # КРИТИЧНО: Паттерн "ом[а]?" был слишком широким и срабатывал на "комната", "дома", "том" и т.д.
        # Теперь используем только явные физические термины
        physics_electric_patterns = [
            r"(?:график\s+)?закон\s+ом[а]?",  # Только "закон Ома" или "график закона Ома"
            r"сила\s+тока\s+от\s+напряжени[я]",  # "сила тока от напряжения"
            r"вольт[-\s]?амперн[ая]?\s+характеристик[аи]",  # "вольт-амперная характеристика"
            r"график\s+сил[ы]?\s+тока\s+от\s+напряжени[я]",  # "график силы тока от напряжения"
            r"напряжение\s+и\s+ток",  # "напряжение и ток"
        ]
        # Отдельные паттерны для электрических схем (требуют явного упоминания электричества)
        # УЛУЧШЕНО: Добавлены детские формулировки
        electric_scheme_patterns = [
            r"электрическ[ая]?\s+схем[аеыу]",  # "электрическая схема"
            r"электрическ[ая]?\s+цеп[ьи]",  # "электрическая цепь"
            r"схем[аеыу]?\s+электрическ[ого]?\s+цеп[и]",  # "схема электрической цепи"
            r"схем[аеыу]?\s+с\s+ламп[ойой]",  # "схема с лампой" (в контексте физики)
            r"схем[аеыу]?\s+цеп[и]",  # "схема цепи"
            r"нарисуй\s+лампочк[у]?\s+и\s+резистор",  # "нарисуй лампочку и резистор"
            r"как\s+соединить\s+проводник",  # "как соединить проводники"
        ]
        # УЛУЧШЕНО: Паттерны для физики - тепловые процессы
        physics_thermal_patterns = [
            r"график\s+нагревани[я]?\s+вод[ы]",  # "график нагревания воды"
            r"когда\s+лед\s+тает",  # "когда лед тает"
            r"график\s+плавлени[я]",  # "график плавления"
            r"крив[ая]?\s+нагрева",  # "кривая нагрева"
        ]

        # Сначала проверяем явные запросы на электрические схемы
        # ТОЛЬКО если есть запрос визуализации И явное упоминание электричества
        if has_visualization_request:
            for pattern in electric_scheme_patterns:
                if re.search(pattern, text_lower):
                    # Электрическая схема цепи
                    image = self.viz_service.generate_electric_circuit_scheme()
                    if image:
                        logger.info("📈 Детектирована электрическая схема цепи")
                        return image, "scheme"
                    # Если не удалось сгенерировать схему цепи, пробуем график закона Ома
                    image = self.viz_service.generate_ohms_law_graph()
                    if image:
                        logger.info("📈 Детектирована электрическая схема/график закона Ома")
                        return image, "graph"

            # Затем проверяем запросы на график закона Ома (только явные физические термины)
            ...
            # Проверяем запросы на тепловые процессы (графики нагревания, плавления)
            for pattern in physics_thermal_patterns:
                if re.search(pattern, text_lower):
                    ...
                    if re.search(r"охлаждени|остывани", text_lower):
                        image = self.viz_service.generate_heating_cooling_graph("cooling")
                    elif re.search(r"плавлени|тает|тающ", text_lower):
                        image = self.viz_service.generate_melting_graph(substance)
                    else:
                        image = self.viz_service.generate_heating_cooling_graph("heating")

                    if image:
                        logger.info(f"📈 Детектирован график теплового процесса: {substance}")
                        return image, "graph"

        # 38. Математика: графики функций (линейная, парабола, синус, косинус и т.д.)
        # ТОЛЬКО если есть явный запрос визуализации
        math_graph_patterns = [
            # Линейная функция (y = kx + b) - ПРИОРИТЕТ!
            r"график\s+линейн[ойая]?\s+функци",  # "график линейной функции"
            r"линейн[ая]?\s+функци[яю]?\s+график",  # "линейная функция график"
            r"прямо\s*[-]?\s*пропорциональн[ая]?\s+(?:зависимость|функци)",  # "прямо пропорциональная"
            r"обратно\s*[-]?\s*пропорциональн[ая]?\s+(?:зависимость|функци)",  # "обратно пропорциональная"
            r"график\s+y\s*=\s*k?x\s*[\+\-]?\s*b?",  # "y = kx + b" или "y = x"
            r"график\s+y\s*=\s*\d*x\s*[\+\-]\s*\d+",  # "y = 2x + 3"
            r"y\s*=\s*kx\s*\+\s*b",  # явное "y = kx + b"
            r"график\s+прям[ойая]",  # "график прямой"
            # Квадратичная функция (парабола)
            r"график\s+(?:парабол|квадратичн)",
            r"график\s+y\s*=\s*x\^2",  # парабола
            r"график\s+y\s*=\s*a?x\^?2",  # ax^2
            r"квадратичн[ая]?\s+функци",  # "квадратичная функция"
            # Тригонометрия
            ...
            r"график\s+y\s*=\s*1/x",  # гипербола
            r"график\s+y\s*=\s*sqrt",  # корень
            r"график\s+y\s*=\s*\|?x\|?",  # модуль
            # Названия функций
            r"парабол[аы]",  # парабола
            r"синусоид[аы]",  # синусоида
            r"гипербол[аы]",  # гипербола
        ]
        ...
        if has_visualization_request or has_context_pattern:
            for pattern in math_graph_patterns:
                if re.search(pattern, text_lower):
                    # Сначала проверяем специфические типы функций по ключевым словам

                    # ЛИНЕЙНАЯ ФУНКЦИЯ (y = kx + b)
                    if re.search(
                        r"линейн|прямо\s*[-]?\s*пропорц|y\s*=\s*k?x\s*[\+\-]|прям[ойая]", text_lower
                    ):
                        # Пытаемся извлечь коэффициенты из текста
                        ...
                        if linear_match:
                            ...
                            b = (
                                linear_match.group(2).replace(" ", "")
                                if linear_match.group(2)
                                else ""
                            )
                            if k == "" or k == "-":
                                k = "-1" if k == "-" else "1"
                            ...
                            function_expr = "2*x + 1"  # Пример по умолчанию
                    ...
                    if re.search(r"обратно\s*[-]?\s*пропорц|гипербол", text_lower):
                        image = self.viz_service.generate_function_graph("1/x")
                        if image:
                            logger.info(
                                "📈 Детектирован график обратной пропорциональности (гиперболы)"
                            )
                            return image, "graph"
                    ...
                    if "синус" in text_lower or "синусоид" in text_lower:
                        ...
                        image = self.viz_service.generate_function_graph("tan(x)")
                        if image:
                    ...
                    if re.search(r"корен|sqrt|корн", text_lower):
                        image = self.viz_service.generate_function_graph("sqrt(x)")
                        if image:
                    ...
                    if re.search(r"модул|\|x\|", text_lower):
                        image = self.viz_service.generate_function_graph("abs(x)")
                        if image:
                            logger.info("📈 Детектирован график модуля")
                            return image, "graph"

                    # Извлекаем функцию из текста для общего случая
