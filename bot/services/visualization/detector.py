...
class VisualizationDetector:
    ...
    def detect(self, text: str) -> tuple[bytes | None, str | None]:
        ...
        # УЛУЧШЕНО: учитываем ВСЕ слова в запросе (покажи/нарисуй/составь + карту + страну)
        # УЛУЧШЕНО: Добавлены детские формулировки
        # Расширенный список с городами и регионами
        map_keywords = [
        ...
        for pattern in map_keywords:
            match = re.search(pattern, text_lower)
            if match:
                # Извлекаем название страны/города из текста (расширенный список)
                ...
                if country_match:
                    ...
                    if (
                        country_name in ["питер", "петербург", "спб"]
                        or "санкт" in country_name
                        or "петербург" in country_name
                    ):
                        country_name = "санкт-петербург"
                    elif "москв" in country_name:
                        country_name = "москва"
                    elif "европ" in country_name:
                        country_name = "европа"
                    else:
                    ...
                    if image:
                        logger.info(f"🗺️ Детектирована карта для: {country_name}")
                        return image, "map"
                else:
