...
class VisualizationDetector:
    ...
    def detect(self, text: str) -> tuple[bytes | None, str | None]:
        ...
        # УЛУЧШЕНО: учитываем ВСЕ слова в запросе (покажи/нарисуй/составь + карту + страну/город/район)
        # УЛУЧШЕНО: Добавлены детские формулировки и запросы на города/районы
        # КРИТИЧЕСКИ ВАЖНО: При запросе города/района показываем ВЕСЬ объект с границами
        # Расширенный список с городами, районами и регионами
        map_keywords = [
            # Запросы на карты стран, городов, районов
            ...
            r"(?:покажи|нарисуй|создай|составь|построй|выведи|отобрази)\s+(?:карт[аеыу]|где\s+находится|местоположени)",
            r"где\s+находится\s+(?:япони|росси|кита|сша|франци|германи|великобритани|инди|бразили|австрали|канад|итали|испани|египт|мексик|страна|европ|турци|польш|украин|казахстан|узбекистан|монголи|таиланд|вьетнам|индонези|филиппин|малайзи|сингапур|саудовск|израил|иран|ирак|пакистан|бангладеш|афганистан|аргентин|чили|перу|колумби|венесуэл|куб|южн[ая]?\s+африк|нигери|кени|эфиопи|марокк|алжир|тунис|ливи|судан|питер|петербург|санкт[-\s]?петербург|спб|москв)",
            r"местоположени[ея]\s+(?:япони|росси|кита|сша|франци|германи|великобритани|инди|бразили|австрали|канад|итали|испани|египт|мексик|европ|турци|польш|украин|казахстан|узбекистан|монголи|таиланд|вьетнам|индонези|филиппин|малайзи|сингапур|саудовск|израил|иран|ирак|пакистан|бангладеш|афганистан|аргентин|чили|перу|колумби|венесуэл|куб|южн[ая]?\s+африк|нигери|кени|эфиопи|марокк|алжир|тунис|ливи|судан|питер|петербург|санкт[-\s]?петербург|спб|москв)",
        ...
        for pattern in map_keywords:
            match = re.search(pattern, text_lower)
            if match:
                # УЛУЧШЕНО: Извлекаем название страны/города/района из текста
                # Приоритет: район > город > страна
                # Сначала проверяем запросы на районы (они более специфичны)
                district_match = re.search(
                    r"(московский|адмиралтейский|василеостровский|выборгский|калининский|кировский|колпинский|красногвардейский|красносельский|кронштадтский|курортный|невский|петроградский|петродворцовый|приморский|пушкинский|фрунзенский|центральный)\s*(?:район)?\s*(?:спб|петербург|питер|москв)?",
                    text_lower,
                )
                if district_match:
                    # Найден район - формируем полное название
                    district_name = district_match.group(1)
                    # Определяем город по контексту
                    if any(word in text_lower for word in ["спб", "петербург", "питер"]):
                        country_name = f"{district_name} район спб"
                    elif "москв" in text_lower:
                        country_name = f"{district_name} район москвы"
                    else:
                        # По умолчанию считаем районом СПб
                        country_name = f"{district_name} район спб"
                ...
                # Затем проверяем города (СПБ, Москва)
                ...
                if city_match:
                    ...
                    if (
                        city_name in ["питер", "петербург", "спб"]
                        or "санкт" in city_name
                        or "петербург" in city_name
                    ):
                ...
                if country_match:
                    ...
                    # Нормализуем названия стран
                    if "европ" in country_name:
                        country_name = "европа"
                    else:
                        # Пробуем найти полное название страны в тексте
                ...
                # Если ничего не найдено, но есть запрос на карту - показываем карту мира
        ...
        for pattern in history_patterns:
            if re.search(pattern, text_lower):
                # Для карт и схем битв пока нет методов - используем хронологическую таблицу
        ...
        # УЛУЧШЕНО: Добавлены паттерны из примера
        # УЛУЧШЕНО: Добавлены детские формулировки
        # TODO: Добавить метод генерации климатограмм (generate_climatogram)
        # Паттерны: r"климатограмм[аеыу]", r"построй\s+климатограмм[аеыу]", r"график\s+температур[ы]?\s+и\s+осадк[ов]",
        # r"климатограмм[аеыу]?\s+та[йи]г[и]", r"температур[аеы]?\s+летом", и т.д.

        # 36.8. Информатика: блок-схемы
        # УЛУЧШЕНО: Добавлены паттерны из примера
        # УЛУЧШЕНО: Добавлены детские формулировки
        # TODO: Добавить метод генерации блок-схем (generate_flowchart)
        # Паттерны: r"блок[-\s]?схем[аеыу]", r"схем[аеыу]?\s+алгоритм[а]", r"цикл\s+в\s+схем[е]",
        # r"ветвлени[ея]", r"таблиц[аеыу]?\s+истинност[и]?\s+и", r"логик[аеыу]?\s+или", и т.д.
        ...
        if has_visualization_request:
            ...
            # УЛУЧШЕНО: Проверяем запросы на тепловые процессы (графики нагревания, плавления)
            for pattern in physics_thermal_patterns:
                if re.search(pattern, text_lower):
                    # Для тепловых процессов можно использовать линейный график с объяснением
                    # Пока используем общий линейный график, в будущем можно добавить специализированный
                    # Для "когда лед тает" - это график плавления (горизонтальная линия при 0°C)
                    # TODO: Добавить специализированный график плавления
                    # Пока просто пропускаем - в будущем добавим генерацию
                    pass
