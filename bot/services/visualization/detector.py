...
class VisualizationDetector:
    ...
    def detect(self, text: str) -> tuple[bytes | None, str | None]:
        ...
        # 36.6. Биология: схемы строения (кровообращение, клетка, органы)
        # УЛУЧШЕНО: Добавлены паттерны из примера для биологических схем
        # УЛУЧШЕНО: Добавлены детские формулировки
        # TODO: Добавить методы генерации биологических схем (generate_biology_cell_structure, generate_biology_circulation)
        # Пока детектируем, но не генерируем (методы будут добавлены позже)
        # Паттерны: r"схем[аеыу]?\s+кровообращени[яе]", r"схем[аеыу]?\s+строени[яе]?\s+клетк[и]",
        # r"внутренност[и]?\s+человек", r"схем[аеыу]?\s+дыхани[яе]", r"решетк[аеыу]?\s+пеннет", и т.д.
        ...
        if has_visualization_request:
            for pattern in electric_scheme_patterns:
                if re.search(pattern, text_lower):
                    # Электрическая схема - пока генерируем график закона Ома
