...
def detect_subject_tables_and_diagrams(
    ...
    if re.search(r"(?:табл[иы]ц[аеы]?\s+)?(?:свойств[а]?\s+корн|квадратн[ый]?\s+корн)", text_lower):
    ...
    # 8c. Таблица значений квадратных корней
    if re.search(r"(?:список|таблиц[аеы]?)\s*(?:значений?\s+)?квадратн\w*\s*корн", text_lower):
        image = viz_service.generate_square_roots_values_table()
        if image:
            logger.info("📊 Детектирована таблица значений квадратных корней")
            return image, "table"
