...
from bot.models import AnalyticsAlert, User, ParentalSettings
...
class AlertService:
    ...
    async def check_all_alerts(self) -> Dict[int, List[AnalyticsAlert]]:
        ...
        try:
            ...
            logger.info(f"🔍 Проверено алертов для {len(active_users)} пользователей: {sum(len(alerts) for alerts in all_alerts.values())} новых")
    ...
    async def _check_alert_rule(self, user: User, rule: AlertRule) -> Optional[AnalyticsAlert]:
        ...
        try:
            ...
            if alert:
                ...
                logger.info(f"🚨 Создан алерт {rule.alert_type.value} для пользователя {user.telegram_id}")
    ...
    async def _evaluate_condition(self, user: User, rule: AlertRule) -> Tuple[bool, Dict[str, Any]]:
        ...
        if rule.alert_type == AlertType.SAFETY_VIOLATION:
            ...
            blocked_count = self.db.query(func.count()).select_from(
    ...
    async def send_alert_notification(self, alert: AnalyticsAlert) -> bool:
        ...
        try:
            ...
            logger.info(f"📤 Отправка уведомления родителю {alert.parent_telegram_id}: {alert.alert_message}")
    ...
    async def get_alert_statistics(self, days: int = 30) -> Dict[str, Any]:
        ...
        try:
            ...
            # Общая статистика
            total_alerts = self.db.query(func.count(AnalyticsAlert.id)).filter(
                AnalyticsAlert.triggered_at >= start_date
            ).scalar()
            
            resolved_alerts = self.db.query(func.count(AnalyticsAlert.id)).filter(
                and_(
                    AnalyticsAlert.triggered_at >= start_date,
            ...
            alerts_by_type = self.db.query(
                AnalyticsAlert.alert_type,
                func.count(AnalyticsAlert.id)
            ).filter(
                AnalyticsAlert.triggered_at >= start_date
            ).group_by(AnalyticsAlert.alert_type).all()
            
            # Статистика по уровням
            alerts_by_level = self.db.query(
                AnalyticsAlert.alert_level,
                func.count(AnalyticsAlert.id)
            ).filter(
                AnalyticsAlert.triggered_at >= start_date
            ).group_by(AnalyticsAlert.alert_level).all()
    ...
    async def _is_alert_in_cooldown(self, user_id: int, alert_type: AlertType) -> bool:
        ...
        try:
            ...
            recent_alert = self.db.query(AnalyticsAlert).filter(
                and_(
                    or_(
    ...
    async def cleanup_old_alerts(self, days: int = 90):
        ...
        try:
            ...
            deleted_count = self.db.query(AnalyticsAlert).filter(
                and_(
