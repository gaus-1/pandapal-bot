        ...
        with self._task_lock:
            return {
                **self._stats,
                ...
                "active_tasks": len([t for t in self._tasks.values() if t.status == TaskStatus.PROCESSING]),
    ...
    def _execute_task(self, task: AsyncTask):
        ...
        try:
            ...
            if task.retry_count <= task.max_retries:
                logger.warning(f"⚠️ Ошибка задачи {task.id}, попытка {task.retry_count}/{task.max_retries}: {e}")
...
def async_task(priority: TaskPriority = TaskPriority.NORMAL, max_retries: int = 3, ttl: Optional[int] = None):
    """
    ...
    def cleanup_old_messages(user_id: int, older_than_days: int = 30):
        ...
        with get_db() as db:
            deleted_count = db.query(ChatHistory).filter(
                ChatHistory.user_telegram_id == user_id,
