...
from bot.models import User, ChatHistory, UserProgress, LearningSession
...
class AnalyticsService:
    ...
    async def get_parent_dashboard(
        ...
        """
        cache_key = cache_service.generate_key("parent_dashboard", parent_id, child_id, period.value)
    ...
    async def _get_user_data(self, user_id: int, start_date: datetime, end_date: datetime) -> Dict[str, Any]:
    ...
    def _build_activity_summary(self, analytics: UserAnalytics) -> Dict[str, Any]:
        ...
        return {
            ...
            "engagement_level": "high" if analytics.engagement_score > 0.7 else "medium" if analytics.engagement_score > 0.4 else "low",
    ...
    def _build_learning_summary(self, analytics: UserAnalytics) -> Dict[str, Any]:
        ...
        return {
            ...
            "top_subject": max(analytics.learning_progress.items(), key=lambda x: x[1]["points"])[0] if analytics.learning_progress else None
        }
    ...
    def _build_safety_summary(self, analytics: UserAnalytics) -> Dict[str, Any]:
        ...
        return {
            ...
            "safety_level": "excellent" if analytics.safety_score > 0.9 else "good" if analytics.safety_score > 0.7 else "needs_attention",
            "risk_level": "low" if analytics.safety_score > 0.8 else "medium" if analytics.safety_score > 0.6 else "high"
        }
    ...
    def _build_charts_data(self, user_id: int, start_date: datetime, end_date: datetime) -> Dict[str, Any]:
    ...
    async def _calculate_trend(self, metric: str, start_date: datetime, end_date: datetime) -> Dict[str, Any]:
