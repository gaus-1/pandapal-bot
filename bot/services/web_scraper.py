...
class WebScraperService:
    ...
    def __init__(self):
        ...
        self.nsportal_config = {
            ...
            "subjects": [
                "matematika", "russkiy-yazyk", "literatura", "istoriya",
                "geografiya", "biologiya", "fizika", "khimiya", "informatika"
            ]
        }
        ...
        self.school203_config = {
            ...
            "sections": [
                "uchebnaya-deyatelnost", "vospitatelnaya-rabota",
                "dopolnitelnoe-obrazovanie", "metodicheskaya-rabota"
            ]
        }
    ...
    async def scrape_nsportal_tasks(self, limit: int = 50) -> List[EducationalContent]:
        ...
        try:
            ...
            for subject in self.nsportal_config["subjects"]:
                ...
                subject_tasks = await self._scrape_nsportal_subject(subject_url, subject, limit // len(self.nsportal_config["subjects"]))
    ...
    async def _scrape_nsportal_subject(self, url: str, subject: str, limit: int) -> List[EducationalContent]:
    ...
    async def _extract_nsportal_material(self, url: str, subject: str) -> Optional[EducationalContent]:
    ...
    async def scrape_school203_content(self, limit: int = 30) -> List[EducationalContent]:
        ...
        try:
            ...
            for section in self.school203_config["sections"]:
                ...
                section_materials = await self._scrape_school203_section(section_url, section, limit // len(self.school203_config["sections"]))
    ...
    async def _scrape_school203_section(self, url: str, section: str, limit: int) -> List[EducationalContent]:
        ...
        try:
            async with self.session.get(url) as response:
                if response.status == 200:
                    ...
                    articles = soup.find_all(['article', 'div'], class_=re.compile(r'(article|post|content)'))[:limit]
    ...
    async def _extract_school203_material(self, article_elem, section: str) -> Optional[EducationalContent]:
        ...
        try:
            ...
            async with self.session.get(news_url) as response:
                if response.status == 200:
                    ...
                    # Ищем новости
                    news_items = soup.find_all(['article', 'div'], class_=re.compile(r'(news|article|post)'))[:20]
        ...
        try:
            ...
            async with self.session.get(news_url) as response:
                if response.status == 200:
                    ...
                    # Ищем новости
                    news_items = soup.find_all(['article', 'div'], class_=re.compile(r'(news|article|post)'))[:15]
    ...
    async def _extract_news_item(self, item_elem, source: str) -> Optional[NewsItem]:
        ...
        try:
            ...
            # Извлекаем контент
            content_elem = item_elem.find(['p', 'div'], class_=re.compile(r'(content|text|summary)'))
            if not content_elem:
            ...
            url = urljoin(self.nsportal_config["base_url"] if "nsportal" in source else self.school203_config["base_url"], 
                         url_elem['href']) if url_elem else ""
    ...
    async def get_educational_knowledge_base(self) -> Dict[str, List[EducationalContent]]:
        ...
        try:
            ...
            logger.info(f"📚 База знаний собрана: {len(all_materials)} материалов по {len(knowledge_base)} предметам")
