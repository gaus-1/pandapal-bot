...
def remove_duplicate_text(text: str, min_length: int = 20) -> str:
    ...
    # НЕ удаляем: ### заголовки, **жирный**, *курсив*, - списки
    # Это позволяет фронтенду красиво отображать структурированные ответы
    #
    # Единственное что делаем - преобразуем markdown в читаемый plain-text формат
    # для Telegram (который не поддерживает markdown заголовки):
    #
    # ### Заголовок → \n\n📌 ЗАГОЛОВОК\n
    result = re.sub(r"^#{1,6}\s+(.+)$", r"\n\n📌 \1\n", result, flags=re.MULTILINE)
    # **жирный текст** → сохраняем как есть (Telegram поддерживает)
    # *курсив* → сохраняем как есть
    # - списки → • для красоты
