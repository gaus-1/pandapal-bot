    ...
    async def generate_response(
        ...
        try:
            ...
            if is_history_cleared and not user_name and not skip_name_asking:
                enhanced_system_prompt += (
                    "\n\nВАЖНО: История чата была очищена (пользователь очистил чат). "
            ...
            # Если пользователь поздоровался или история пустая - можно поприветствовать
            if (user_greeted or is_history_cleared) and not chat_history:
                enhanced_system_prompt += (
