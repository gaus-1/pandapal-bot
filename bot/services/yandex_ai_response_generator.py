...
def remove_duplicate_text(text: str, min_length: int = 20) -> str:
    ...
    # Убираем курсив *text* (только одиночные, не в словах)
    result = re.sub(r"(?<!\w)\*(.+?)\*(?!\w)", r"\1", result)
    # Убираем курсив _text_ НО сохраняем математические индексы (log_a, x_1, a_n)
    # Математические индексы: буква/цифра + _ + буква/цифра (без пробелов)
    # НЕ удаляем: log_a, x_1, a_n, sin_x и т.д.
    # Удаляем только: _курсив_ (с пробелами внутри или длинный текст)
    result = re.sub(r"(?<![a-zA-Z0-9])_([^_\s]{2,}[^_]*?)_(?![a-zA-Z0-9])", r"\1", result)
...
class YandexAIResponseGenerator:
    ...
    async def generate_response(
        ...
        try:
            ...
            # КРИТИЧЕСКИ ВАЖНО: НЕ упоминать источники (Wikipedia, ссылки и т.д.)
            additional_context = ""
            if verified_context:
                ...
                additional_context += (
                    "КРИТИЧЕСКИ ВАЖНО: НИКОГДА не упоминай источники информации (Wikipedia, ссылки, "
                    "сайты и т.д.) в ответе. Отвечай от своего имени, как будто это твои знания."
                )
