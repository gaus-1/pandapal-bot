...
class PromptBuilder:
    ...
    def build_system_prompt(
        ...
        prompt += self._get_strict_structure_instruction(user_message=user_message)

        return prompt

    def _get_strict_structure_instruction(self, user_message: str = "") -> str:
        """
...
Что это: (если нужно объяснить понятие)
Краткое описание (1-2 предложения).

Как читать/использовать:
- Конкретный пример 1
- Конкретный пример 2
- Конкретный пример 3

Основные свойства: (если применимо)
- Свойство 1
- Свойство 2
...
Для графиков: укажи точки, возрастание/убывание, примеры.
Для карт: положение, соседи, водные объекты, факты.
НИКОГДА не используй символы ** для форматирования.
