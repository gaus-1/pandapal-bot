...
@module bot.services.advanced_moderation
"""
...
class AdvancedModerationService:
    ...
    def _build_category_patterns(self) -> Dict[ContentCategory, List[Pattern]]:
        ...
        patterns[ContentCategory.VIOLENCE] = [
            re.compile(r'\b(убить|убийство|смерть|умереть|труп|кровь|ранен|бьют|драка|война|оружие|нож|пистолет|бомба|взрыв)\b', re.IGNORECASE),
            re.compile(r'\b(избить|изнасиловать|пытка|мучение|страдание|боль|убийца|преступник)\b', re.IGNORECASE),
            re.compile(r'\b(самоубийство|суицид|вешаться|отравиться|резать|резаться)\b', re.IGNORECASE),
        ]
        
        # Наркотики
        patterns[ContentCategory.DRUGS] = [
            re.compile(r'\b(наркотик|наркота|героин|кокаин|амфетамин|марихуана|гашиш|спайс|соль|кристалл)\b', re.IGNORECASE),
            re.compile(r'\b(наркоман|зависимость|доза|инъекция|курение|нюхать|колоться)\b', re.IGNORECASE),
            re.compile(r'\b(трава|косяк|джойнт|булька|спиды|экстази|лсд|мескалин)\b', re.IGNORECASE),
        ]
        
        # Сексуальный контент
        patterns[ContentCategory.SEXUAL] = [
            re.compile(r'\b(секс|порно|интим|голый|голая|раздеться|лечь|трахать|ебать|порнография)\b', re.IGNORECASE),
            re.compile(r'\b(оргазм|возбуждение|мастурбация|педофил|изнасилование|проституция)\b', re.IGNORECASE),
        ]
        
        # Политика и экстремизм
        patterns[ContentCategory.POLITICS] = [
            re.compile(r'\b(путин|зеленский|байден|трамп|выборы|голосование|партия|политика|власть)\b', re.IGNORECASE),
            re.compile(r'\b(революция|переворот|протест|митинг|демонстрация|бунт|мятеж)\b', re.IGNORECASE),
        ]
        
        patterns[ContentCategory.EXTREMISM] = [
            re.compile(r'\b(фашизм|нацизм|терроризм|ислам|джихад|расизм|ксенофобия|геноцид)\b', re.IGNORECASE),
            re.compile(r'\b(скинхед|неонацист|террорист|боевик|радикал|экстремист)\b', re.IGNORECASE),
        ]
        
        # Буллинг
        patterns[ContentCategory.BULLYING] = [
            re.compile(r'\b(дурак|тупой|идиот|дебил|кретин|придурок|неудачник|лузер|толстый|урод)\b', re.IGNORECASE),
            re.compile(r'\b(ненавижу|презираю|убил бы|убью|убейся|сдохни|покончи с собой)\b', re.IGNORECASE),
        ]
        
        # Мошенничество
        patterns[ContentCategory.SCAM] = [
            re.compile(r'\b(деньги|заработать|доход|прибыль|инвестиции|криптовалюта|биткоин|майнинг)\b', re.IGNORECASE),
            re.compile(r'\b(скидка|акция|бесплатно|подарок|выигрыш|приз|лотерея|казино)\b', re.IGNORECASE),
        ]
        
        # Спам
        patterns[ContentCategory.SPAM] = [
            re.compile(r'\b(реклама|продажа|купить|заказать|доставка|скидка|промокод|акция)\b', re.IGNORECASE),
            re.compile(r'\b(подписка|регистрация|ссылка|сайт|телефон|звонок|сообщение)\b', re.IGNORECASE),
        ]
    ...
    def _build_intent_indicators(self) -> Dict[str, List[str]]:
        ...
        return {
            "educational": ["изучать", "узнать", "понять", "объяснить", "научить", "учебник", "урок", "задача"],
    ...
    async def moderate_content(self, content: str, user_context: Dict[str, Any] = None) -> ModerationResult:
        """
    ...
    def _check_category(self, content: str, category: ContentCategory, patterns: List[Pattern]) -> Optional[Dict]:
    ...
    def _analyze_intent(self, content: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "is_harmful": intent_scores.get("harmful", 0) > 0 or intent_scores.get("dangerous", 0) > 0,
            "is_educational": intent_scores.get("educational", 0) > 0
        }

    def _make_decision(self, category_results: List[Dict], intent_analysis: Dict, 
                      context_analysis: Dict, user_context: Dict[str, Any] = None) -> ModerationResult:
    ...
    def _get_suggested_action(self, level: ModerationLevel, category: ContentCategory, confidence: float) -> str:
    ...
    def _get_alternative_response(self, category: ContentCategory, context_analysis: Dict) -> Optional[str]:
