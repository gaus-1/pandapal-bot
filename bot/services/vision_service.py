...
class VisionService:
    ...
    async def generate_educational_response(
        ...
        try:
            ...
            logger.info(
                f"📚 Сгенерирован образовательный ответ для изображения: {analysis_result.category.value}"
            )
    ...
    async def create_learning_task_from_image(
        ...
        try:
            ...
            task = {
                ...
                "expected_answers": self._generate_expected_answers(
                    analysis_result, detected_subject
                ),
            ...
            return {"error": "Не удалось создать образовательную задачу", "reason": str(e)}
    ...
    async def _apply_moderation(
        self, result: ImageAnalysisResult, user_message: Optional[str]
    ) -> ImageAnalysisResult:
    ...
    def _build_educational_prompt(
        ...
        subjects_text = (
            ", ".join(analysis_result.subjects_detected)
            if analysis_result.subjects_detected
            else "общие знания"
        )
    ...
    def _generate_expected_answers(
        self, analysis_result: ImageAnalysisResult, subject: str
    ) -> List[str]:
    ...
    def _is_age_appropriate(
        self, analysis_result: ImageAnalysisResult, user_age: Optional[int]
    ) -> bool:
