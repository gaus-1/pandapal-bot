    ...
    async def check_homework(
        ...
        try:
            ...
            homework_system_prompt = (
                ...
                "Тон: дружелюбный и честный, без иронии в оценке. "
                f"{FORMATTING_RULES_SNIPPET} "
                "Проверь ДЗ, укажи ошибки и объясни. При разборе — примеры правильного решения. "
                "Не упоминай источники. Решай задачи полностью, не только подсказывай."
            )
            result = await self.yandex_service.analyze_image_with_text(
                image_data=image_data,
                user_question=homework_prompt,
