...
def get_diagram_explanation(
    visualization_type: str, location_name: str | None = None
) -> str | None:
    """Получить подробное пояснение для типа визуализации.

    Args:
        visualization_type: Тип визуализации (bar, pie, map и т.д.).
        location_name: Для карт — название объекта (опционально).
    """
    if visualization_type == "map" and location_name:
        base = (
            f"🗺️ На карте выше показан(а) **{location_name}**! "
            "Обрати внимание на масштаб — он помогает понять размеры территории. "
            "Метка в центре указывает на запрошенный объект. "
            "Границы и дороги на карте показывают административное деление. "
            "Это изучается в географии, 5–9 класс, тема «Картография и ориентирование»."
        )
    else:
