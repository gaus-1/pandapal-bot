...
async def miniapp_ai_chat_stream(request: web.Request) -> web.StreamResponse:
    ...
    try:
        ...
        with get_db() as db:
            ...
            # КРИТИЧЕСКИ ВАЖНО: Дополнительное напоминание о запрете $ и LaTeX И СТРУКТУРЕ
            enhanced_system_prompt += (
                "\n\n🚫🚫🚫 КРИТИЧЕСКОЕ НАПОМИНАНИЕ - ОБЯЗАТЕЛЬНО К ВЫПОЛНЕНИЮ: "
                ...
                "\n\n📋 КРИТИЧЕСКИ ВАЖНО - СТРУКТУРА ОТВЕТОВ: "
                "ВСЕГДА структурируй ответы с переносами строк! "
                "Каждый шаг решения - с новой строки! "
                "Каждый пункт списка - с новой строки! "
                "НЕ пиши все в одной строке полотном текста! "
                "Используй пустые строки для разделения блоков! "
            ...
            if should_greet:
                ...
                # КРИТИЧЕСКИ ВАЖНО: Если панда уже поздоровалась, НЕ повторяй приветствие!
                # Даже если пользователь поздоровался - панда уже поздоровалась, отвечай по делу!
                if ai_already_greeted:
                    enhanced_system_prompt += (
                        "\n\n🚫🚫🚫 КРИТИЧЕСКИ ВАЖНО: Панда УЖЕ поздоровалась ранее (в истории есть приветствие от панды). "
                        "Пользователь может поздороваться, но ты УЖЕ поздоровалась - НЕ повторяй приветствие!\n"
                        "❌ НЕ говори 'Привет' в начале ответа!\n"
                        "❌ НЕ говори 'Привет! Чем могу помочь?'!\n"
                        "❌ НЕ говори 'Я рад помочь тебе'!\n"
                        "❌ НЕ говори 'Рад помочь'!\n"
                        "❌ НЕ говори 'С удовольствием помогу'!\n"
                        "❌ НЕ говори 'Конечно, помогу'!\n"
                        "✅ ОТВЕЧАЙ СРАЗУ ПО ДЕЛУ на вопрос пользователя!\n"
                        "✅ Если пользователь просто поздоровался - ответь кратко и спроси чем помочь, БЕЗ повторения 'Привет'!\n"
