...
export const LegalPageLayout: React.FC<LegalPageLayoutProps> = React.memo(
  ({ title, subtitle, children }) => {
    return (
      <>
        <Header />
        <main className="max-w-3xl mx-auto px-3 xs:px-4 sm:px-6 md:px-8 pt-6 xs:pt-7 sm:pt-8 pb-4 sm:pb-6 md:pb-8 w-full box-border">
          <article className="bg-white dark:bg-slate-800 rounded-xl xs:rounded-2xl sm:rounded-3xl border border-gray-100 dark:border-slate-700 dark:border-slate-600/50 shadow-md dark:shadow-lg p-4 xs:p-5 sm:p-6 md:p-7 lg:p-8">
            ...
            <div className="font-sans text-xs sm:text-sm text-gray-700 dark:text-slate-200 leading-[1.75] space-y-4 sm:space-y-5 max-w-none [&_h2]:font-display [&_h2]:font-bold [&_h2]:text-sm md:[&_h2]:text-base [&_h2]:text-gray-900 dark:[&_h2]:text-slate-50 [&_h2]:mt-6">
              {children}
            </div>
          </article>
        </main>
