...
export const PrivacyPage: React.FC = React.memo(() => {
  return (
    ...
    >
      ...
      <h2 className="font-display font-bold text-sm sm:text-base text-gray-900 dark:text-slate-50 mt-fib-3">
      ...
      <p>
        Идентификатор Telegram, имя и имя пользователя (username), данные обратной связи (если указаны), технические данные (IP, тип устройства), данные, необходимые для приёма оплаты (в соответствии с политикой платёжного провайдера). На сайте могут использоваться cookie и метрика для анализа посещаемости. В том числе используется Яндекс.Метрика для анализа посещаемости.
      </p>
