      ...
      if (error instanceof DOMException) {
        if (error.name === 'NotAllowedError' || error.name === 'PermissionDeniedError') {
          ...
          if (error.message.includes('system') || error.message.includes('Permission denied by system')) {
            // Это известная проблема Telegram WebView на Android
            const platform = telegram.getPlatform();
            if (platform === 'android' || platform === 'weba') {
              errorMessage = (
                '⚠️ Голосовые сообщения временно недоступны на твоем устройстве.\n\n' +
                'Это ограничение Telegram WebView на Android.\n\n' +
                '💡 Что можно сделать:\n' +
                '• Используй текстовый ввод для общения с Пандой\n' +
                '• Отправляй голосовые сообщения через обычный чат Telegram бота\n' +
                '• Попробуй открыть Mini App в десктопной версии Telegram\n\n' +
