        ...
        <main className="max-w-4xl mx-auto px-4 sm:px-6 md:px-8 py-6 sm:py-8 md:py-10">
          <article className="rounded-2xl sm:rounded-3xl bg-white/95 dark:bg-slate-800/95 border border-gray-100 dark:border-slate-600/60 p-5 sm:p-7 md:p-8 shadow-lg dark:shadow-xl">
            <header className="mb-5 sm:mb-6">
              ...
              <p className="mt-2 text-xs sm:text-sm text-gray-600 dark:text-slate-300">{subtitle}</p>
            </header>

            <section className="space-y-4">
              <p className="text-xs sm:text-sm md:text-base text-gray-700 dark:text-slate-200 leading-[1.72]">
                {description}
              </p>
              ...
              <div className="space-y-4">
                {faq.map((item) => (
                  <div key={item.question} className="rounded-xl border border-gray-200 dark:border-slate-600 p-4 sm:p-5">
                    <h3 className="font-semibold text-xs sm:text-sm text-gray-900 dark:text-slate-100 mb-1.5">{item.question}</h3>
                    <p className="text-xs sm:text-sm text-gray-700 dark:text-slate-200 leading-[1.7]">{item.answer}</p>
                  </div>
                ))}
              </div>
            </section>
