...
- **SOLID SRP** — `database.py`, `cache_service.py`, `detector.py` вынесены в пакеты; данные отделены от логики (`adult_topics_data.py`); хелперы из `ai_chat/text.py` — в `helpers.py`
...
**Статистика тестов:**
- 🧪 **Всего тестов: 525+**
- ✅ **Unit тесты: 60+** (безопасность, SSRF, audit logging, БД, кеш, модерация)
