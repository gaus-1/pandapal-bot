...
**PandaPal** — это современная образовательная платформа, объединяющая Telegram-бота и веб-приложение для безопасного и эффективного обучения школьников 1-9 классов.
...
- ✅ **150+ фильтров контента** — многоуровневая система модерации
- ✅ **Модерация 24/7** — автоматическая проверка всех сообщений
- ✅ **Мягкое перенаправление** — запрещенные темы блокируются без упоминания
- ✅ **Фильтрация по возрасту** — контент адаптируется под класс ребенка
- ✅ **OWASP Top 10 compliance** — защита от основных уязвимостей

### 📱 **Telegram Mini App**
- ✅ **Полнофункциональное веб-приложение** внутри Telegram
- ✅ **Кроссплатформенность** — работает на ПК (web.telegram.org) и мобильных
- ✅ **Аутентификация через Telegram** — безопасная валидация initData
- ✅ **Адаптивный UI** — пастельные цвета, удобный интерфейс для детей
- ✅ **PWA поддержка** — работает как нативное приложение

### 🎓 **Адаптивность и образование**
- ✅ **Контент подстраивается под возраст** — 1-9 класс (6-15 лет)
- ✅ **Адаптивные AI промпты** — сложность ответов зависит от класса
- ✅ **Упрощенные формулы** — химические уравнения и математика через простые слова и таблицы
- ✅ **Поддержка всех школьных предметов** — математика, физика, химия, биология и др.

### 🎨 **Мультимодальность**
- ✅ **Текст** — полноценный AI чат с генерацией ответов
- ✅ **Голос** — распознавание речи через STT (Speech-to-Text)
- ✅ **Изображения** — анализ фото через OCR (Optical Character Recognition)
- ✅ **Решение задач с фото** — математика, физика, химия и другие школьные предметы
- ✅ **Реальная обработка медиа** — не имитация, полная интеграция с AI сервисами

### 🛠️ **Современный технологический стек**

#### **Backend:**
- ✅ **Python 3.13** — последняя версия Python
- ✅ **aiogram 3.x** — современный Telegram Bot API фреймворк
- ✅ **SQLAlchemy 2.0** — async ORM с type hints
- ✅ **Pydantic V2** — валидация данных
- ✅ **PostgreSQL 17** — реляционная БД
- ✅ **FFmpeg** — конвертация аудио (WebM → OggOpus)
...
### 🇷🇺 **Российский AI**
- ✅ **Генерация ответов** — AI модели для русского языка
- ✅ **Распознавание речи** — STT (Speech-to-Text) для русской речи
- ✅ **Распознавание текста** — OCR (Optical Character Recognition) на изображениях
- ✅ **Полная интеграция** — все сервисы через облачные AI API

### 💎 **Качественный код**
- ✅ **Соответствие PEP 20** — код следует принципам Zen of Python
- ✅ **Модульная архитектура** — SOLID принципы, разделение ответственности
- ✅ **447+ тестов** — покрытие критических компонентов 90%+
- ✅ **Идиоматичный Python** — современные паттерны и best practices
- ✅ **Pre-commit hooks** — автоматическая проверка качества (Black, isort, flake8)
- ✅ **Type hints** — полная типизация для надежности
...
- ![aiohttp](https://img.shields.io/badge/-aiohttp-2C5F77?logo=aiohttp&logoColor=white) — асинхронный веб-сервер (webhook + frontend)
...
- ![FFmpeg](https://img.shields.io/badge/-FFmpeg-007808?logo=ffmpeg&logoColor=white) — конвертация аудио (WebM → OggOpus)
...
- ![TanStack Query](https://img.shields.io/badge/-TanStack%20Query-FF4154?logo=reactquery&logoColor=white) — серверное состояние и кеширование
- ![Zustand](https://img.shields.io/badge/-Zustand-443B48?logo=zustand&logoColor=white) — клиентское состояние
- ![Telegram WebApp](https://img.shields.io/badge/-Telegram%20WebApp-2CA5E0?logo=telegram&logoColor=white) — Telegram Mini App SDK

### **AI & External APIs**
- **Облачные AI сервисы** — генерация текста, распознавание речи и изображений
...
│                     │                           │    │
│  ┌──────────────────▼─────┐   ┌─────────────────▼─┐  │
│  │   PostgreSQL 17        │   │  AI Cloud Services│  │
│  │   (Railway Managed)    │   │  - Text Generation│  │
│  │   - Connection Pool│ │  - Speech Recognition│ │
│  │   - SSL/TLS            │   │  - Image Recognition│ │
│  └────────────────────────┘   └───────────────────┘  │
│                                                      │
...
- **Unified Service** — бот + frontend + Mini App в одном контейнере (экономия ресурсов)
- **Hybrid Architecture** — один кодбейс для лендинга и Mini App
- **Database Isolation** — PostgreSQL в отдельном managed сервисе
- **SOLID** — все сервисы следуют принципам
- **Security-First** — безопасность на каждом уровне (initData валидация)
- **PEP 20 Compliance** — код следует принципам Zen of Python
- **Modern Frontend** — TanStack Query, Zustand, lazy loading, code splitting
...
**Последние обновления (2-3 января 2026):**
- ✅ Telegram Mini App полностью функционален (ПК и мобильные)
- ✅ Упрощенные формулы и таблицы для детей (без LaTeX)
- ✅ Мягкое перенаправление с запрещенных тем (без упоминания темы)
- ✅ Улучшенная обработка assets файлов (универсальный handler)
- ✅ Подавление ошибок Telegram WebView
- ✅ Reply keyboard удалена (все функции в Mini App)
- ✅ Обновлены SEO мета-теги и structured data (JSON-LD)

#### **🤖 Telegram Бот**
- ✅ Текстовые запросы с AI ответами
- ✅ Голосовые сообщения (STT - распознавание речи)
- ✅ Аудиофайлы (распознавание речи, WebM → OggOpus через FFmpeg)
- ✅ Анализ изображений (OCR - распознавание текста)
- ✅ Решение задач с фото (все школьные предметы)
- ✅ Поддержка всех школьных предметов (1-9 класс)
- ✅ Экстренные номера России
- ✅ Модерация контента 24/7 (мягкое перенаправление без упоминания запрещенных тем)
- ✅ Адаптивные промпты для каждого класса
- ✅ Упрощенные формулы и таблицы для детей (без LaTeX)
- ✅ Reply keyboard удалена — все функции в Mini App

#### **📱 Telegram Mini App**
- ✅ Полнофункциональный AI чат с фото и аудио
- ✅ Реальная обработка медиа (OCR для изображений, STT для аудио)
- ✅ Экстренные номера с возможностью звонка
- ✅ Аутентификация через Telegram initData (строгая валидация)
- ✅ Адаптивный UI с пастельными цветами
- ✅ Lazy loading компонентов для оптимизации
- ✅ TanStack Query для кеширования и синхронизации
- ✅ Zustand для глобального состояния
- ✅ PWA поддержка (Service Worker отключен в Telegram WebView)
- ✅ Работает на ПК (web.telegram.org) и мобильных устройствах
- ✅ Универсальный обработчик assets для корректной раздачи файлов
- ✅ Подавление ошибок Telegram WebView (SW, device-orientation)
...
#### **🌐 Веб-приложение (Landing + Mini App)**
- ✅ Адаптивный дизайн (mobile-first)
- ✅ PWA (Progressive Web App) с Service Worker (отключен в Telegram)
- ✅ SEO оптимизация (обновленные мета-теги, JSON-LD, sitemap.xml)
- ✅ Dark/Light режимы
- ✅ Lazy loading компонентов
- ✅ Веб-аналитика интеграция (SSR поддержка)
- ✅ Гибридная архитектура: лендинг в браузере, Mini App в Telegram
- ✅ Structured Data (JSON-LD) для поисковых систем
- ✅ Обновленный sitemap.xml с актуальными датами
...
├── 📁 frontend/                 # React приложение (Landing + Mini App)
│   ├── src/
│   │   ├── components/          # UI компоненты (лендинг)
│   │   ├── features/             # Фичи (AIChat, Emergency)
│   │   ├── services/             # API клиенты (telegram, api)
│   │   ├── hooks/                # Custom hooks (useAuth, useChat)
│   │   ├── store/                # Zustand store
│   │   ├── lib/                  # Утилиты (queryClient)
│   │   ├── MiniApp.tsx           # Главный компонент Mini App
│   │   └── App.tsx               # Корневой компонент (роутинг)
│   └── public/                   # Статика (logo.png, sw.js, offline.html)
...
**Покрытие:**
- **Unit тесты:** Zustand store, custom hooks (useAuth, useChat)
- **Integration тесты:** MiniApp компонент, AIChat критичные сценарии
- **E2E тесты:** Playwright для полного пользовательского флоу
- **83/100 тестов проходят** (остальные в разработке)
...
- **Веб-аналитика** — отслеживание пользователей с SSR поддержкой
- **Coverage Reports** — HTML/XML/Terminal
- **Pre-commit hooks** — автоматические проверки (Black, isort, flake8)
- **Analytics Service** — автоматическая запись бизнес-метрик в PostgreSQL
- **Playwright** — E2E тестирование с реальным браузером
- **Vitest** — быстрые unit и integration тесты для React
...
- **AI:** Облачные AI сервисы (генерация текста, распознавание речи и изображений)
- **Infrastructure:** Railway.app (webhook mode, 24/7 uptime)

### **Статистика проекта:**
- **Коммитов:** 150+
- **Строк кода:** 25,000+
- **Тестов:** 447 (backend) + 83 (frontend) = 530+
- **Покрытие:** 53% (backend), 83% (frontend critical paths)
- **Mini App:** Полностью функциональный с реальной обработкой медиа
- **AI:** Генерация текста, распознавание речи (STT) и изображений (OCR)
- **Формулы:** Упрощенные для детей (без LaTeX, через таблицы и простые слова)
- **Безопасность:** 150+ фильтров, мягкое перенаправление
