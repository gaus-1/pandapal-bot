...
PandaPal — безопасный AI-помощник и робот-репетитор для школьников 1–9 класса в формате Telegram Mini App. Помогает с домашними заданиями по всем предметам, использует YandexGPT, обрабатывает фото и голос, работает 24/7. Модерация контента и родительский контроль.
...
- **Визуализации по всем предметам** — графики функций, таблицы умножения/сложения/деления, таблица значений квадратных корней (√1–√50), карты стран и городов с границами, климатограммы природных зон, блок-схемы алгоритмов, графики плавления и нагревания, периодическая таблица Менделеева
...
- **Голосовые вопросы** — распознавание речи через SpeechKit STT с подтверждением («Отправить» / «Исправить») перед отправкой в AI
...
- Параметры генерации: temperature=0.4, max_tokens=8192
...
│   │   │   ├── text.py      # Текстовые сообщения (оркестратор-pipeline)
...
│   │   ├── cache/                       # Кеширование (пакет, SOLID SRP)
│   │   │   ├── memory.py               # MemoryCache (LRU)
│   │   │   ├── service.py              # CacheService (Redis + Memory), cached декоратор
│   │   │   └── specialized.py          # UserCache, ModerationCache, AIResponseCache
│   │   ├── embeddings_service.py        # Векторные эмбеддинги (Yandex Embeddings API)
│   │   ├── miniapp/                     # Сервисы Mini App (пакет)
│   │   │   ├── chat_context_service.py  # Контекст чата
│   │   │   ├── intent_service.py        # Детекция intent'ов (таблицы, графики и т.п.)
│   │   │   ├── audio_service.py         # Обработка аудио
│   │   │   ├── photo_service.py         # Обработка фото/домашки
│   │   │   └── visualization_service.py # Детектор визуализаций
│   │   ├── games_service/               # Игры PandaPalGo (пакет, mixin-архитектура)
│   │   │   ├── session.py              # CRUD сессий, статистика, достижения
│   │   │   ├── tic_tac_toe.py          # Крестики-нолики
│   │   │   ├── checkers.py             # Шашки
│   │   │   ├── game_2048.py            # 2048
│   │   │   └── erudite.py              # Эрудит
│   │   ├── game_engines/                # Движки игр
│   │   │   ├── tic_tac_toe.py          # TicTacToe engine
│   │   │   ├── checkers.py             # Checkers engine
│   │   │   ├── game_2048.py            # 2048 engine
│   │   │   └── erudite.py              # Erudite engine
│   │   ├── visualization_service.py     # Единая точка входа для визуализаций
│   │   ├── visualization/               # Генерация графиков/таблиц по предметам
│   │   │   ├── detector.py              # Оркестратор детекции визуализаций
│   │   │   ├── detectors/               # Модули детекции (SOLID SRP)
│   │   │   │   ├── request_words.py     # Триггерные слова
│   │   │   │   ├── schemes.py           # Схемы (астрономия, биология, ДНК)
│   │   │   │   ├── diagrams.py          # Диаграммы (bar, pie, line, scatter)
│   │   │   │   ├── maps.py              # Карты (районы, города, страны)
│   │   │   │   ├── physics.py           # Графики физики
│   │   │   │   ├── math_graphs.py       # Графики мат. функций
│   │   │   │   └── tables_and_diagrams.py # Предметные таблицы (в т.ч. √1–√50)
│   │   │   ├── math/                    # Математические визуализации
│   │   │   ├── sciences/                # Физика, химия
│   │   │   ├── social/                  # География, история
│   │   │   ├── languages/               # Таблицы грамматики
...
│   │   ├── speech_service.py                # Распознавание голоса (SpeechKit STT)
...
│   │   │   ├── stream_handlers/ # Модули streaming-обработки (пакет)
│   │   │   │   ├── ai_chat_stream.py  # Основной оркестратор SSE
│   │   │   │   ├── _pre_checks.py     # Валидация запроса, Premium-проверки
│   │   │   │   ├── _media.py          # Обработка аудио/фото
│   │   │   │   ├── _routing.py        # Маршрутизация (модерация, темы, изображения)
│   │   │   │   ├── _visualization.py  # Генерация визуализаций
│   │   │   │   ├── _fallback.py       # Fallback-pipeline без streaming
│   │   │   │   ├── _history.py        # Сохранение истории, геймификация
│   │   │   │   └── _utils.py          # Общие утилиты
│   │   │   ├── homework.py      # Проверка домашних заданий (check, history, statistics)
│   │   │   ├── chat.py          # Non-streaming AI чат
...
│       ├── sql_migrations.py # SQL миграции (premium, payments)
...
- **Модульная структура** — крупные файлы разбиты на пакеты и подмодули (< 500 строк)
- **SOLID SRP** — `database/`, `cache/`, `games_service/`, `visualization/detectors/`, `stream_handlers/` вынесены в пакеты; данные отделены от логики (`adult_topics_data.py`); хелперы из `ai_chat/text.py` — в `helpers.py`
- **Mixin-архитектура** — `GamesService` собирается из базового класса + отдельных mixin'ов по играм; `stream_handlers/` разбит на 7 модулей-хелперов
- **SOLID OCP/DIP** — новые детекторы визуализаций добавляются без изменения оркестратора; интерфейсы сервисов через `interfaces.py`
- **PEP 20** — явные импорты, плоская структура пакетов, обратная совместимость через re-export shims
...
- **Устойчивость** — Circuit Breaker для Yandex GPT/STT/Vision, retry при таймаутах (tenacity)
- **Масштабирование** — горизонтально: один webhook (один активный приёмник или балансировщик); FSM и кеш в Redis
...
- `games_service/` — логика игр PandaPalGo (пакет: TicTacToe, Checkers, 2048, Erudite)
- `gamification_service.py` — достижения, уровни, XP
- **`visualization/`** — Генерация визуализаций по предметам:
  - `detector.py` — оркестратор детекции визуализаций (~300 строк)
  - `detectors/` — модули детекции (SOLID SRP): `request_words`, `schemes`, `diagrams`, `maps`, `physics`, `math_graphs`, `tables_and_diagrams`
...
- `handlers/ai_chat/` — модульная структура AI чата:
  - `text.py` — обработка текстовых сообщений (оркестратор-pipeline)
  - `voice.py` — голосовые и аудио сообщения с FSM-подтверждением («Отправить» / «Исправить») перед отправкой в AI
  - `image.py` — анализ изображений
  - `document.py` — обработка документов
  - `helpers.py` — вспомогательные функции (Premium-лимиты, визуализации, перевод, отправка ответов, фидбек)
  - `__init__.py` — регистрация router и handlers

### API Endpoints

- `bot/api/miniapp/chat_stream.py` → `stream_handlers/ai_chat_stream.py` — Mini App AI chat с streaming (SSE), голос, изображения, визуализации (разбит на 7 модулей)
- `bot/api/miniapp/chat.py` — Non-streaming AI чат
- `bot/api/miniapp/homework.py` — проверка домашних заданий (check, history, statistics)
- `bot/api/miniapp/progress.py` — прогресс обучения
...
**Статистика тестов:**
- 🧪 **Всего тестов: 1000+**
- ✅ **Unit тесты: 60+ файлов** (безопасность, SSRF, audit logging, БД, кеш, модерация, игры, сервисы)
- ✅ **Integration тесты: 38+ файлов** (API, платежи, криптография, реальный Yandex Cloud API)
- ✅ **E2E тесты: 5 файлов** (полные пользовательские сценарии)
- ✅ **Security тесты: 6 файлов** (OWASP, SQL injection, DDoS, авторизация)
- ✅ **Resilience тесты: 5 файлов** (устойчивость компонентов, деградация сервисов)
- ✅ **Performance тесты: 7 файлов** (нагрузка endpoints, БД, платежи)
...
**Пример проверки (security тесты):**
- ✅ Тесты авторизации корректно возвращают 403 — **подтверждение работы A01**
...
- `YANDEX_CLOUD_API_KEY`, `YANDEX_CLOUD_FOLDER_ID` — для YandexGPT, SpeechKit, Vision, Embeddings API
...
- **Форма обратной связи**: `offer_feedback_form` использует `total_requests` вместо `message_count`
- **Голос (FSM)**: подтверждение распознанного текста — inline-кнопки «Отправить» / «Исправить» перед отправкой в AI
- **Embeddings API**: `get_embedding` в YandexCloudService, EmbeddingService, таблица `embedding_cache` (pgvector)
...
- **`bot/services/games_service.py`** (1025 строк) → пакет `bot/services/games_service/`: `session.py`, `tic_tac_toe.py`, `checkers.py`, `game_2048.py`, `erudite.py`; mixin-архитектура с `GamesServiceBase` + game-specific mixins; facade через `__init__.py`
- **`bot/api/miniapp/stream_handlers/ai_chat_stream.py`** (1743 строк) → оркестратор + 7 модулей: `_pre_checks.py`, `_media.py`, `_routing.py`, `_visualization.py`, `_fallback.py`, `_history.py`, `_utils.py`
- **`bot/database.py`** (633 строк) → пакет `bot/database/`: `engine.py`, `alembic_utils.py`, `sql_migrations.py`, `service.py`; backward-compatible re-exports через `__init__.py`
- **`bot/services/cache_service.py`** (652 строк) → пакет `bot/services/cache/`: `memory.py`, `service.py`, `specialized.py`; shim для обратной совместимости
...
- **Enhanced RAG система** — полноценная система Retrieval-Augmented Generation
  ...
  - **Semantic Cache** — векторный кеш через pgvector + Yandex Embeddings API (cosine similarity), fallback при недоступности API
  - **Context Compression** — сжатие контекста на 75–90%, защита списков при запросах «список»/«таблица значений»
